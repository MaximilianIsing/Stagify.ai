...
/**
 * Furniture per room: `[symbol, fx, fy, widthMetres, depthMetres]`, where fx/fy are the
 * item's CENTRE as a fraction of the room. Sizes are real metres rather than fractions
 * because the rooms are different shapes — a sofa scaled to "20% of the room" would be a
 * different sofa in every room, which is the tell that gives away a fake plan.
 *
 * Exported so the test can assert that every room in the markup has an entry, that every
 * symbol it names exists in index.html's <defs>, that nothing overhangs a wall, and that
 * nothing intrudes into the label band at the top of the room.
 *
 * @type {Record<string, Array<[string, number, number, number, number]>>}
 */
export const FURNITURE = {
  basics: [
    ['fp-sofa', 0.3, 0.51, 2.4, 0.9], ['fp-rug', 0.3, 0.78, 2.6, 1.3],
    ['fp-chair', 0.62, 0.52, 0.8, 0.8], ['fp-lamp', 0.62, 0.85, 0.55, 0.55],
  ],
  turnaround: [
    ['fp-desk', 0.28, 0.5, 2, 0.85], ['fp-chair', 0.28, 0.75, 0.75, 0.75],
    ['fp-shelf', 0.6, 0.88, 1.8, 0.6], ['fp-lamp', 0.62, 0.47, 0.55, 0.55],
  ],
  pricing: [
    ['fp-table', 0.55, 0.52, 1.8, 1], ['fp-counter', 0.55, 0.9, 1.6, 0.6],
    ['fp-plant', 0.87, 0.52, 0.6, 0.6], ['fp-lamp', 0.87, 0.85, 0.5, 0.5],
  ],
  studios: [
    ['fp-sofa', 0.56, 0.5, 2.2, 0.9], ['fp-round', 0.56, 0.81, 1.2, 1.2],
    ['fp-shelf', 0.22, 0.9, 1.3, 0.55], ['fp-lamp', 0.87, 0.85, 0.5, 0.5],
  ],
  control: [
    ['fp-desk', 0.5, 0.44, 1.9, 0.8], ['fp-chair', 0.5, 0.55, 0.75, 0.75],
    ['fp-sofa', 0.5, 0.68, 2.2, 0.9], ['fp-shelf', 0.5, 0.81, 1.8, 0.6],
    ['fp-plant', 0.85, 0.93, 0.85, 0.85],
  ],
  photos: [
    ['fp-bed', 0.28, 0.6, 2, 2.2], ['fp-shelf', 0.28, 0.9, 2, 0.6],
    ['fp-chair', 0.62, 0.45, 0.8, 0.8], ['fp-lamp', 0.85, 0.45, 0.55, 0.55],
    ['fp-round', 0.8, 0.8, 1.1, 1.1],
  ],
  disclosure: [
    ['fp-table', 0.55, 0.52, 1.6, 0.9], ['fp-chair', 0.55, 0.83, 0.7, 0.7],
    ['fp-shelf', 0.86, 0.55, 1.2, 0.55], ['fp-lamp', 0.86, 0.88, 0.5, 0.5],
  ],
  privacy: [
    ['fp-desk', 0.55, 0.52, 1.8, 0.8], ['fp-chair', 0.55, 0.83, 0.7, 0.7],
    ['fp-shelf', 0.86, 0.55, 1.2, 0.55], ['fp-plant', 0.3, 0.88, 0.6, 0.6],
  ],
  whyStagify: [
    ['fp-sofa', 0.5, 0.44, 2.2, 0.9], ['fp-round', 0.5, 0.64, 1.3, 1.3],
    ['fp-counter', 0.5, 0.9, 2.2, 0.7], ['fp-chair', 0.85, 0.55, 0.8, 0.8],
    ['fp-lamp', 0.15, 0.8, 0.55, 0.55],
  ],
};

/**
 * A room's outline, clockwise from the top-left — the direction a hand traces a wall,
 * which is what makes the draw-on read as drafting rather than as a progress bar.
...
/**
 * Outline length, for `--len`. Plain arithmetic on purpose: `getTotalLength()` forces
 * layout, and doing it once per room on a section nobody has scrolled to is real cost
 * for a number that is 2(w+h).
...
function mountRoom(room) {
  ...
  const wall = svg('path', { class: 'fp-wall', d: rectPath(w, h) });
  wall.style.setProperty('--len', String(round(perimeter(w, h))));
  ...
  if (items.length) {
    kids.push(svg('g', { class: 'fp-furn' }, items.map(([symbol, fx, fy, wmItem, hmItem]) => {
      const iw = wmItem * U;
      const ih = hmItem * U;
      return svg('use', {
        href: `#${symbol}`,
