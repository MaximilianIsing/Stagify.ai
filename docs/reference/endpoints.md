...
| `GET` | `/api/gallery/:id/source` | The render's `after` bytes, served **by this process** as `image/webp`. Exists for one caller: "Refine in Masking Studio". That studio draws the photo onto a canvas and then calls `toDataURL` on it — and **drawing a cross-origin image taints a canvas, so `toDataURL` throws `SecurityError`**. A presigned R2 URL is cross-origin, so handing one over would break every generate and every export the studio does; worse, dev and CI serve blobs same-origin (`routes/object-local.js`), so it would pass every test and fail only in production. The client fetches this with its bearer token (which an `<img src>` could not carry) and makes a blob URL, which taints nothing. Ownership is checked before a byte is read, and "not yours" is the same `404` as "does not exist". **This is a deliberate exception** to the rule that render bytes never pass through this process — one ~110 KB WebP, once, for the authenticated owner, on an explicit click. Do not widen it into a general "serve my render" route. |
...
| `POST` | `/api/masking-studio/save` | **The Masking Studio's "Looks Good" → a gallery entry.** **Auth:** **`requireProAccount`**. The only gallery writer that generates nothing — the composite is made on a canvas in the browser and posted back, which is why it exists at all. **Body (JSON):** `after` (required data URL, the composite), `before` (optional data URL, the pristine original), `renderId` (optional), `areas`, `prompts[]`, `sourceName`. **Two outcomes, and the client does not get to choose:** with a `renderId` the handler asks the store to **replace** that render's bytes in place — `replaceResult` has ownership, `evicted_at IS NULL` and `status = 'ok'` in its WHERE, so a guessed or stale id simply fails to match — and **anything else inserts** a new entry named `Masking Studio — N areas`. A replace keeps the entry's id, `created_at`, `custom_name`, `extra_json` and share link, so a refined *interior* render stays "Luxury Bedroom" rather than being renamed after the studio. **No metering:** the generations were already billed at `/api/mask-edit`, one per painted area. **Limiters:** `galleryImportLimiter` (20 / 15 min, `RL_GALLERY_IMPORT`) **and** `genLimiter` — the dedicated one because this is the only endpoint that accepts megabytes of client pixels and PUTs them to object storage with no model call in between to brake it. **Body size:** registered in `JSON_LARGE_LIMIT_PATHS`; without that it 413s before the handler runs. **Success:** `{ success, replaced, gallery }`, where `gallery` is `null` on a replace (nothing was created) and on a deployment with no object store. |
