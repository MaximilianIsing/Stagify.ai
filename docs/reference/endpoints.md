...
| `GET` | `/api/admin/api-usage` | **API usage tab.** `?days=N` (clamped 1–90, default 30) → `{ usage }`, or `{ usage: null, reason: 'unavailable' }` when no reader is configured — it degrades rather than 503ing, like `/api/admin/metrics`. One pre-aggregated site-wide read of the public render API: UTC-day buckets of delivered/refunded requests, a per-account breakdown joined to the account email, prepaid credit economics (sold vs. granted vs. burned, outstanding liability, suspended accounts) and key counts. A bad `days` is **clamped, not rejected** — it is an operator typing in a URL bar. Returns no key ids, key prefixes, idempotency keys or fingerprints; it is a shaped aggregate and never touches an individual request row. Carries customer emails and spend, so it is `no-store`. Statements are prepared once at factory time; see the N+1 guard in `test/analytics/api-usage.test.js`. |
