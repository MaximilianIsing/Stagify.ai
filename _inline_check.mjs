...
;
      ...
      async function ensureDesignerProAccess() {
        // Non-Pro and anonymous users are sent to the homepage's AI Designer demo
        // row rather than seeing this Stagify+-only tool.
      ...
      // Pro gate + welcome message when page loads
          ...
          <div class="mask-editor-content">
            ...
            <div class="mask-editor-controls">
              ...
              <div class="mask-editor-tools" role="group" aria-label="Mask tool">
                ...
                <p class="mask-editor-note" data-i18n="pdf.maskEditor.brushAllNote">Be sure to brush over all of the area you'd like masked.</p>
              </div>
              ...
              <div class="mask-editor-ref-container">
                ...
                <input type="file" id="mask-editor-ref-file" accept="image/jpeg,image/png,image/webp,image/jpg">
      ...
      function updateMaskEditorTranslations() {
        ...
        // Update the "brush over all" note
        const note = document.querySelector('.mask-editor-note');
        if (note) {
          note.textContent = getText('pdf.maskEditor.brushAllNote');
        }
      ...
      // ── Mask compositing helpers (mirror the main Stagify mask tool) ──
      // Grow the brushed area outward by `grow` px (silent brush expansion).
      function maskGrowBinary(drawSrc, w, h, grow) {
      ...
      // White-on-black mask sent to the model (the grown editable region).
      function maskBuildModelMask(drawSrc, w, h, grow) {
        const grown = maskGrowBinary(drawSrc, w, h, grow);
      ...
      // Feathered alpha mask for compositing (grown core + soft falloff).
      function maskBuildBlendMask(drawSrc, w, h, coreGrow, featherPx) {
        const grown = maskGrowBinary(drawSrc, w, h, coreGrow + featherPx);
        const out = document.createElement('canvas');
        out.width = w; out.height = h;
        const octx = out.getContext('2d');
        let blurred = false;
        ...
        } catch (err) { blurred = false; }
        if (!blurred) {
          octx.drawImage(grown, 0, 0);
          const steps = 28, rings = 8;
      ...
      // Hard-composite: keep the original everywhere, paste the edit only inside
      // the feathered mask, so unbrushed areas can never change and there's no seam.
      ...
      function maskCompositeEdit(origCanvas, keepMask, editedImg, w, h) {
        return maskCompositeEditCanvas(origCanvas, keepMask, editedImg, w, h).toDataURL('image/png');
      }
      ...
      function maskSetPhase(p) {
        ...
        if (p === 'refine') {
          ...
          if (help) {
            ...
            if (tip) tip.textContent = lang('pdf.maskEditor.refineHelp', "This step just fine-tunes where the AI's change shows — it doesn't run the AI again. Brush to reveal more of the edit, erase to pull it back. It's a safety net so the edit only touches the area you picked and can't mess up the rest of your photo.");
          }
          maskRecolor('#16a34a');
          if (note) note.textContent = lang('pdf.maskEditor.refineNote', "Brush to reveal more of the edit, erase to hide it — this only re-crops, it won't re-run the AI.");
          updateApplyButtonState();
        } else {
          ...
          if (note) note.textContent = lang('pdf.maskEditor.brushAllNote', "Be sure to brush over all of the area you'd like masked.");
        }
      }
      ...
      function maskRenderRefinePreview() {
        ...
        // Ghost the FULL raw AI output on top at 40% so the user can see the entire
      ...
      function initMaskDrawing(maskCanvas) {
        ...
        function pointFrom(e) {
          const cv = document.getElementById('mask-editor-canvas');
          const sx = parseFloat(cv.dataset.scaleX) || 1;
          const sy = parseFloat(cv.dataset.scaleY) || 1;
