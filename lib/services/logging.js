...
const PROMPT_LOG_HEADER = 'timestamp,roomType,furnitureStyle,additionalPrompt,removeFurniture,userRole,referralSource,email,ipAddress,status,durationMs,model,attempts,errorCode,architectureDrift,seed,variation';
// Every header this file has ever written, oldest first, kept only so an existing log can be
// upgraded in place. A LIST rather than a single constant: this used to be one string, so
// the second time a column was appended the upgrade would have silently no-opped on any log
// still carrying the first-generation header, leaving it mislabelled forever. Append to this
// array whenever PROMPT_LOG_HEADER grows; never edit an entry, and never reorder.
const PROMPT_LOG_HEADERS_LEGACY = [
...
export function createLogging(deps) {
  ...
  function upgradePromptLogHeader(logFile) {
    ...
    try {
      ...
      if (brk === -1) return;
      if (!PROMPT_LOG_HEADERS_LEGACY.includes(text.slice(0, brk).replace(/\r$/, ''))) return;
      const tmp = logFile + '.tmp';
      fs.writeFileSync(tmp, PROMPT_LOG_HEADER + '\n' + text.slice(brk + 1));
      fs.renameSync(tmp, logFile);
      logger.info('[logging] Upgraded prompt_logs.csv header to the current column set');
    } catch (e) {
 ...
 * @param {boolean | null} [outcome.architectureDrift] - Whether the QA reviewer judged the delivered render to have changed the room's architecture. Omit/null when the question was not asked.
 * @param {number | null} [outcome.seed] - The base generation seed, so a reported bad render can be reproduced.
 * @param {string | null} [outcome.variation] - Which variation of how many, as `"n/N"`, when one request fanned out into several renders. Omit/null for a render that was not one of a set.
 */
function logPromptToFile(promptText, roomType, furnitureStyle, additionalPrompt, removeFurniture, userRole, userReferralSource, userEmail, req, outcome) {
  try {
    ...
    const csvRow = [
      ...
      // 'yes' / 'no' / '' — empty means the question was never asked (quality gate off, or
      // a path with no source photo to compare against), which is NOT the same as "clean".
      // Conflating the two would report a reviewer outage as a perfect drift rate.
      escapeCsvField(typeof o.architectureDrift === 'boolean' ? (o.architectureDrift ? 'yes' : 'no') : ''),
      // The base seed the render used. Its value means nothing on its own — it is here so a
      // "this render bricked up my window" report can be re-run instead of argued about.
      escapeCsvField(typeof o.seed === 'number' ? String(o.seed) : ''),
      // "n/N" for one of a multi-variation request's renders, '' for everything else.
      // Blank means "this render was not one of a set", NOT "it was the first" — the
      // browser handler tags even a single render '1/1', so a blank here is a chat,
      // Exterior or API render rather than a studio one.
      escapeCsvField(typeof o.variation === 'string' ? o.variation : '')
    ].join(',') + '\n';
...
function logRejectionToFile(kind, code, detail, who = {}) {
  try {
    // An explicit null is as common as an omitted argument at the anonymous call
    // sites, and it must land in the same place: a row that says "unknown", not a
    // swallowed TypeError that drops the rejection entirely.
    const actor = who || {};
    const req = actor.req || null;
    const timestamp = new Date().toISOString();
    const ipAddress = req ? (req.ip || req.connection?.remoteAddress || 'unknown') : 'unknown';
    ...
    const csvRow = [
      ...
      escapeCsvField(actor.email || 'unknown'),
      escapeCsvField(actor.userId || 'unknown'),
      escapeCsvField(ipAddress),
      escapeCsvField(userAgent),
    ].join(',') + '\n';
