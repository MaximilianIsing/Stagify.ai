...
// cleaning + downscaling), the DEBUG-only outgoing-payload logging, and the
// routing call itself (runChatRouting — the counterpart to runUploadRouting).
//
// deps: { DEBUG_MODE, downscaleImageForGPT, openai, getTemperatureForModel }
 ...
 * application, OpenAI messages assembly, DEBUG-only payload logging, and the
 * routing call.
 * @param {{ DEBUG_MODE: boolean, downscaleImageForGPT: (url: string) => Promise<string>, openai: any, getTemperatureForModel: (model: string) => number }} deps - Injected dependencies (DEBUG_MODE here is the injected value, not runtime-flags).
 * @returns {{ logDedupDiagnostics: Function, detectHistoryImage: Function, applyMessageTag: Function, buildChatMessages: Function, logChatPayload: Function, runChatRouting: (args: { openaiMessages: any[], selectedModel: string }) => Promise<any> }} The /api/chat pre-routing helper bundle.
 */
export default function createChatRequestPrep(deps) {
  const { DEBUG_MODE, downscaleImageForGPT, openai, getTemperatureForModel } = deps;
  ...
  function applyMessageTag(filteredMessages, messageTag) {
    if (messageTag && messageTag !== 'auto' && filteredMessages.length > 0) {
      ...
      if (lastMessage.role === 'user') {
        const tagText = MESSAGE_TAG_PREFIXES[messageTag] || '';
  ...
  /**
   * Ask the model for a routing decision and unpack it — the JSON-endpoint mirror of
   * chat-upload-prep's runUploadRouting. On success returns the same field bundle that
   * function does; on failure returns `{ routingError }` instead of throwing, because
   * /api/chat answers a model outage with its own 500 body (a user-facing `response`
   * string) rather than the generic one the handler's outer catch would produce.
   *
   * There is no image-format recovery branch here: that retry exists on the upload
   * endpoint only, where an unsupported file can be described back to the model as text.
   * @param {{ openaiMessages: any[], selectedModel: string }} args - The assembled messages and the allow-listed model.
   * @returns {Promise<{ routingError?: any, text?: string, aiResponseJson?: any, memoryActionsFromAI?: any, stagingRequestFromAI?: any, imageRequestFromAI?: any, recallRequestFromAI?: any, generateRequestFromAI?: any, cadRequestFromAI?: any }>} The routing fields, or `{ routingError }` when the call/parse failed.
   */
  async function runChatRouting({ openaiMessages, selectedModel }) {
    let completion;
    let aiResponseJson;
    try {
  ...
  return { logDedupDiagnostics, detectHistoryImage, applyMessageTag, buildChatMessages, logChatPayload, runChatRouting };
}
