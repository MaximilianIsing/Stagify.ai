...
import { DESIGNER_ROUTING_RESPONSE_FORMAT, MESSAGE_TAG_PREFIXES } from '../staging/prompts.js';
import { filterUnsupportedFiles, filterConversationHistory, stripImagesFromHistory } from './chat-history.js';
import { parseDesignerRoutingCompletion } from './chat-routing.js';

/**
 * Build the pre-routing preparation API for the /api/chat-upload endpoint: turn the
 * multipart upload into GPT-ready messages and run the routing completion.
 * @param {{ DEBUG_MODE: boolean, openai: any, annotateImage: (imageDataUrl: string, isCAD?: boolean, detectBlueprint?: boolean) => Promise<string | null>, downscaleImageForGPT: (dataUrl: string) => Promise<string>, getTemperatureForModel: (model: string) => number }} deps - Debug flag, injected OpenAI client (loosely typed), image annotator, image downscaler, and model→temperature resolver.
 * @returns {{ buildUploadUserContent: (args: any) => any, buildUploadMessages: (args: any) => Promise<any>, logUploadPayload: (args: any) => void, runUploadRouting: (args: any) => Promise<any>, logUploadDedupDiagnostics: (originalHistory: any[], deduplicatedHistory: any[]) => void }} The upload-prep API (buildUploadUserContent, buildUploadMessages, logUploadPayload, runUploadRouting, logUploadDedupDiagnostics).
 */
export default function createUploadPrep(deps) {
  ...
  /**
   * DEBUG-only: list which conversation-history messages were dropped as duplicates
   * for the /api/chat-upload endpoint — the multipart mirror of chat-request-prep's
   * logDedupDiagnostics. No-op unless DEBUG_MODE and something was actually removed.
   * Iterates the pre-dedup array directly (the old inline version re-parsed the raw
   * conversationHistory string to recover it; passing it in drops that redundant parse).
   * @param {import('../types/chat.js').ChatMessage[]} originalHistory - History before dedup.
   * @param {import('../types/chat.js').ChatMessage[]} deduplicatedHistory - History after dedup.
   * @returns {void}
   */
  function logUploadDedupDiagnostics(originalHistory, deduplicatedHistory) {
    if (!DEBUG_MODE) return;
    const removedCount = originalHistory.length - deduplicatedHistory.length;
    if (removedCount <= 0) return;
    logger.debug(`[Deduplication] Removed ${removedCount} duplicate message(s) from conversation history (${originalHistory.length} -> ${deduplicatedHistory.length})`);
    // Log which messages were duplicates
    const seenKeys = new Set();
    originalHistory.forEach((msg, idx) => {
  ...
  /**
   * Validate/annotate each uploaded file and build the user-message content array,
   * kicking off image annotations in the background. Unsupported/AVIF files are noted as
   * text instead of being sent to OpenAI.
   * @param {{ files: import('../types/chat.js').UploadedFile[], message: string, messageTag: string }} args - The multipart files, the user's text message, and the client-selected routing tag.
   * @returns {{ userContent: any[], fileInfo: Array<{ name: string, type: string }>, hasImages: boolean, firstImageFile: import('../types/chat.js').UploadedFile | null, unsupportedFiles: Array<{ name: string, type: string, ext?: string, fileType: string }> }} The assembled content plus file metadata and the rejected-file list.
   */
  function buildUploadUserContent({ files, message, messageTag }) {
    ...
    if (message && message.trim()) {
      ...
      if (messageTag && messageTag !== 'auto') {
        messageText = `${MESSAGE_TAG_PREFIXES[messageTag] || ''} ${messageText}`.trim();
      }
      ...
      userContent.push({ type: 'text', text: MESSAGE_TAG_PREFIXES[messageTag] || '' });
    }
    ...
    for (const file of files) {
      ...
      if (file.mimetype.startsWith('image/') && supportedImageTypes.includes(file.mimetype)) {
        ...
        const annotationPromise = annotateImage(imageDataUrl, false, true).then(annotation => {
          if (DEBUG_MODE) {
            logger.debug(`[Image Annotation] Annotation for ${file.originalname}: ${annotation || 'failed'}`);
          }
          return annotation;
        }).catch(err => {
          logger.error(`[Image Annotation] Error annotating ${file.originalname}:`, err);
          return null;
        });
  ...
  /**
   * Filter + strip conversation history, await image annotations/downscaling, and assemble
   * the OpenAI messages array (system + history + current upload).
   * @param {{ systemInstruction: string, userContent: any[], files: import('../types/chat.js').UploadedFile[], conversationHistory: import('../types/chat.js').ChatMessage[] }} args - System prompt, current user content, uploaded files, and prior history.
   * @returns {Promise<{ filteredUserContent: any[], safeMessages: any[], cleanedUserContent: any[] }>} The filtered content, the assembled OpenAI messages, and the cleaned current-message content.
   */
  ...
  /**
   * DEBUG-only: log the outgoing payload size and per-message summaries. No-op unless DEBUG_MODE.
   * @param {{ safeMessages: any[], selectedModel: string, hasImages: boolean }} args - The assembled messages, the selected model id, and whether images are present.
   * @returns {void}
   */
  function logUploadPayload({ safeMessages, selectedModel, hasImages }) {
    ...
    if (DEBUG_MODE) {
      logger.debug('=== SENDING TO AI (CHAT-UPLOAD) ===');
      logger.debug('Payload size:', payloadSize, 'bytes (', payloadSizeKB, 'KB /', payloadSizeMB, 'MB)');
      logger.debug('Model:', selectedModel);
      logger.debug('Has images:', hasImages);
      logger.debug('Number of messages:', safeMessages.length);
    }
  ...
  /**
   * Call OpenAI for the routing decision, with a fallback pass that lets the AI respond
   * about unsupported/invalid image formats. Re-throws non-image errors to the caller.
   * @param {{ safeMessages: any[], selectedModel: string, message: string, unsupportedFiles: any[], conversationHistory: import('../types/chat.js').ChatMessage[], systemInstruction: string }} args - Assembled messages, model id, user message, rejected files, history, and system prompt.
   * @returns {Promise<{ text: string, aiResponseJson: import('../types/chat.js').RoutingDecision, memoryActionsFromAI: import('../types/chat.js').MemoryActions, stagingRequestFromAI: any, imageRequestFromAI: any, recallRequestFromAI: any, generateRequestFromAI: any, cadRequestFromAI: any }>} The parsed routing fields.
   */
  ...
  return { buildUploadUserContent, buildUploadMessages, logUploadPayload, runUploadRouting, logUploadDedupDiagnostics };
}
