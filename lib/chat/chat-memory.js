...
export default function createMemoryDispatch(deps) {
  ...
  function applyMemoryActions({ memoryActionsFromAI, memories, userId, userMessageText }) {
    /** @type {import('../types/chat.js').MemoryActions} */
          ...
          for (const memoryId of memoryActionsFromAI.forgets) {
            ...
            if (memories.length < initialLength) {
              ...
              const memoryToForget = memories.find(m =>
                ...
                (m.id != null && (m.id.includes(memoryId) || memoryId.includes(m.id)))
              );

              if (memoryToForget) {
                memories = memories.filter(m => m.id !== memoryToForget.id);
                // id is set at creation (line ~84) so it's effectively always present;
                // fall back to the requested identifier if a record ever lacks one.
                memoryActions.forgets.push(memoryToForget.id ?? memoryId);
                if (DEBUG_MODE) {
