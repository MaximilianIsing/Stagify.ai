...
export function aiResponseDefersImageAction(responseText) {
  ...
  const deferPatterns = [
    ...
    /\bquick question\b/,
    // The disclosure question, which none of the patterns above reach — they are all about
    // style, colour or which image, and this one is a yes/no offer about the finished file.
    // Without it a model that asks AND sets shouldStage:true bills a render the user was
    // being asked to confirm. The two negative guards at the top of this function run
    // first, so the legitimate "I've staged your room — want the label too?" is an ACTION
    // turn and still renders.
    /\b(virtually staged|disclosure) (label|note|stamp|tag)\b/,
    /\blabel (it|them|these|this|the (photo|image|render))\b.*\bvirtually staged\b/,
  ];
 ...
 * @param {import('../types/chat.js').GenerateRequest | import('../types/chat.js').GenerateRequest[] | null} generateReq - The routing 'generate' request(s).
 * @param {import('../types/chat.js').CadRequest | import('../types/chat.js').CadRequest[] | null} cadReq - The routing 'cad' request(s).
 * @returns {'floorplan' | 'staging' | 'generating' | 'general'} The loading-status category.
 */
export function chatIntentType(stagingReq, generateReq, cadReq) {
  ...
  // CAD gets its own category rather than borrowing 'staging'. It is checked FIRST, so a
  // turn that does both reports the floor-plan work — that is the slow half (a
  // gemini-3-pro-image call, ~30s, against ~8s for a staged photo), and the status line
  // exists to describe the wait. "Staging your room…" for half a minute while a floor plan
  // renders described neither what was happening nor how long it would take.
  if (some(cadReq, 'shouldProcessCAD')) return 'floorplan';
