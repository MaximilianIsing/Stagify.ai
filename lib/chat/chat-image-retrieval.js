...
/**
 * Build the image-retrieval slice of the AI Designer pipeline: pull an already-existing
 * history image by index. `resolveRecalledImage` only displays; `resolveRequestedImage`
 * optionally re-runs GPT to analyze the image.
 * @param {{ DEBUG_MODE: boolean, openai: any, downscaleImageForGPT: (dataUrl: string) => Promise<string>, getTemperatureForModel: (model: string) => number }} deps - Debug flag, injected OpenAI client (loosely typed), image downscaler, and model→temperature resolver.
 * @returns {{ resolveRecalledImage: (args: any) => (string | null), resolveRequestedImage: (args: any) => Promise<{ requestedImageForDisplay: string | null, text: string }> }} The retrieval API.
 */
export default function createImageRetrieval(deps) {
  ...
  /**
   * Recall (retrieve + display, no analysis) a previous history image by index.
   * @param {{ recallRequestFromAI: any, history: import('../types/chat.js').ChatMessage[] }} args - The AI recall request and the conversation history to pull from.
   * @returns {string | null} The recalled image data URL, or null when absent or on failure.
   */
  ...
  /**
   * Handle an image request: always return the image for display, and — when the user asked
   * to describe/analyze it — make a second GPT call and return the new reply text.
   * @param {{ imageRequestFromAI: any, history: import('../types/chat.js').ChatMessage[], baseMessages: any[], systemInstruction: string, userMessageText: string, analysisUserText: string, selectedModel: string, text: string }} args - Image request, history, base messages, prompts, model id, and the current reply text.
   * @returns {Promise<{ requestedImageForDisplay: string | null, text: string }>} The image to display and the (possibly updated) reply text.
   */
