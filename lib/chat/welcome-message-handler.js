...
//
// deps: { openai, requireProAccount, getUserIdentifier, loadMemories }
 ...
 * @param {{ openai: any, requireProAccount: (req: import('express').Request, res: import('express').Response) => any, getUserIdentifier: (req: import('express').Request) => string, loadMemories: (userId: any) => any[] }} deps - Injected OpenAI client (loosely typed; unused when null), pro gate, user-id resolver, and memory loader.
...
export default function createWelcomeMessageHandler(deps) {
  const { openai, requireProAccount, getUserIdentifier, loadMemories } = deps;

  /**
   * Handle GET /api/welcome-message. Pro-gated; responds `{ message, isReturning }`.
   * @param {import('express').Request} req - Express request (`?userId` optional).
