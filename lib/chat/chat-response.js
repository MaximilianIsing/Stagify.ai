...
export async function buildDesignerResponse({
  ...
  if (cadResults.length > 0) {
    // ALWAYS shipped, single result or many — unlike `cadParams`, which is set only in the
    // plural branch below. The browser needs the view to describe the image: a top-down
    // plan render and an eye-level room photo want different alt text, and for a lone
    // render there was previously nothing on the wire to tell them apart.
    response.cadViews = cadResults.map((r) => (r.params?.view === 'eye-level' ? 'eye-level' : 'top-down'));
