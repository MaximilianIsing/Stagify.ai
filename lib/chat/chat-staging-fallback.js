...
export function resolveCurrentUploadFallbackImage({ firstImageFile, message }) {
  if (firstImageFile && !userWantsToAddFurnitureToRoom(message)) {
    return {
      ...
      // The photo's own name, for the gallery entry this render becomes. It is carried
      // ONLY on this branch, and that is the point: this is the one path where the buffer
      // provably came from a file the user just picked. A history image has no filename to
      // speak of, and the furniture-suppressed branch above returns null precisely because
      // the upload is not the room — attaching its name there would label a staged bedroom
      // after a photo of a sofa.
      sourceName: firstImageFile.originalname || '',
    };
  }
  return null;
}
