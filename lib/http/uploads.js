...
// memoryStorage buffers uploads in RAM (no temp files on disk); shared by the two
// configs below. hostImageUpload constructs its own instance, matching the original.
const storage = multer.memoryStorage();

/**
 * The staging upload ceiling, in bytes — the ONE number that actually rejects a file.
 *
 * It used to be a bare literal here while the browser refused at 100 MB and the guides
 * quoted 10 MB, so a 30 MB photo passed the client check, uploaded in full, was refused
 * by multer, and the user was told to "upload an image smaller than 100MB". Three
 * numbers, none of them the real one. The frontend now mirrors this value and
 * test/http/upload-limit-consistency.test.js fails if the two drift apart.
 */
export const MAX_UPLOAD_BYTES = 25 * 1024 * 1024;

/**
 * The MIME types the staging upload accepts. Exported because GET /api/v1/options
 * publishes it: a second hand-written copy in the docs would be wrong the first time
 * this list changed, and the API caller has no file picker to constrain them.
 * Matched on `file.mimetype` alone — the extension is never examined.
 */
export const STAGING_IMAGE_MIME_TYPES = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];

export const imageFileFilter = (req, file, cb) => {
  if (STAGING_IMAGE_MIME_TYPES.includes(file.mimetype)) {
    cb(null, true);
  } else {
