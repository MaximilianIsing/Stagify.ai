 ...
 * `code` and `details` are omitted when falsy so simple errors stay `{ error }`.
 * The client detects failure from the HTTP status, not the body, so callers
 * pass the real status here rather than relying on a body-level ok/success flag.
 */
export function sendError(res, status, error, { code, details } = /** @type {{ code?: string, details?: string }} */ ({})) {
  const body = { error };
