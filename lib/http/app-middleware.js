...
import { createTextAssetMiddleware } from './text-assets.js';
import { logger } from '../logger.js';

// Absolute, so the static mount does not depend on the process's CWD. It only ever
// resolved because both entrypoints (`npm start`, and scripts/start.sh which execs it)
// happen to run from the repo root; anything launching server.js from elsewhere —
// a supervisor, a debugger, a future container WORKDIR — would have served nothing.
// Two levels up: lib/http/ -> repo root. Matches how every other `public/` path in the
// codebase is built (routes/* off the injected __dirname, lib/data/counters.js, …).
...
export function applyEdgeMiddleware(app) {
  ...
  const cspDirectives = {
    ...
    // Two upload-conversion workers ride this. heic2any's is created from a blob: URL;
    // pdf.js's is a same-origin file (public/scripts/vendor/pdf.worker.min.js), which is
    // what 'self' covers — dropping it would make floor-plan PDFs silently hang rather
    // than error, because the failure is a blocked worker and not a failed fetch.
    workerSrc: ["'self'", 'blob:'],
    childSrc: ["'self'", 'blob:'],
    ...
    frameSrc: [
      "'self'",
      // No https://www.instagram.com here on purpose. It was allowlisted for the three
      // post embeds in the homepage's #instagram section; those are local WebP covers
      // linking out to the posts now, so nothing frames Instagram. Re-adding it is a
      // deliberate decision to load Meta code on a page with no consent gate, not a
      // routine unblock — test/frontend/home-instagram.test.js pins the section against it.
      'https://accounts.google.com',
      'https://*.stripe.com',
  ...
  // The escape hatch is one env var away by design — which is exactly why it must not
  // be silent. Flipped on to unblock a resource mid-incident and then forgotten, it
  // leaves the site serving every response with NO policy at all, and nothing in the
  // logs, the headers we control, or the code says so. warn (not info/debug) so it
  // clears production's info floor and is searchable in Render's log view.
  const cspDisabled = process.env.DISABLE_CSP === '1';
  if (cspDisabled) {
    logger.warn(
      '[security] DISABLE_CSP=1 — Content-Security-Policy is OFF for every response. ' +
        'This is a debug-only escape hatch; unset it once the blocked resource is fixed.'
    );
  }
  app.use(
    helmet({
      contentSecurityPolicy: cspDisabled ? false : { directives: cspDirectives },
...
export function applyBodyAndStatic(app) {
  ...
  // COMMENT-STRIPPED .html/.css, ahead of express.static. This codebase documents itself
  // in its source files and that prose is worth keeping — but it is also 60-72% of the
  // weight of the five render-blocking stylesheets, which are the one thing standing
  // between the homepage and its first paint. Stripping at serve time keeps both: the
  // files on disk are unchanged, and the browser is spared 106 KB on the wire and 287 KB
  // of tokenising. It answers only for those two extensions and calls next() for anything
  // it cannot serve, so express.static below stays the source of truth for what exists.
  app.use(createTextAssetMiddleware(PUBLIC_DIR));
