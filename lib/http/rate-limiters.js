 ...
 * @param {{ name: string, windowMs: number, limit: number, message: object, keyGenerator?: (req: any, res: any) => string }} spec - Limiter id recorded as the rejection code, window, ceiling, the JSON body sent when it trips, and an optional bucket key (defaults to express-rate-limit's IP).
 * @returns {import('express').RequestHandler} The configured limiter middleware.
 */
function rejectingLimiter({ name, windowMs, limit, message, keyGenerator }) {
  return rateLimit({
    ...rlOpts,
    windowMs,
    limit,
...
// Masking Studio save (POST /api/masking-studio/save). A dedicated ceiling because this is
// the first endpoint in the app that accepts MEGABYTES OF CLIENT-SUPPLIED PIXELS and PUTs
// them straight to object storage with no model call in between — every other gallery
// writer is gated by the cost and latency of a generation, which is a rate limit of its
// own. That natural brake is absent here, and PRO_GALLERY_LIMIT now defaults to Infinity
// (lib/data/staged-renders.js), so nothing else bounds how much a single account can store.
//
// 20 per 15 minutes is generous against real use: one save per finished composite, and each
// composite costs one /api/mask-edit call per painted area, all of which already spend
// genLimiter's budget. Imported directly by routes/staging.js rather than injected, for the
// same reason validateImageLimiter is — an omitted dep must never silently leave a paid,
// storage-writing endpoint unlimited. Tests override it via the router's dep.
export const galleryImportLimiter = rejectingLimiter({
  name: 'gallery_import',
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.RL_GALLERY_IMPORT || 20),
  message: { error: 'Too many saves. Please wait a few minutes and try again.' },
});
...
// Badge preview (GET /api/disclosure-preview): unauthenticated, and every cold request
// runs a sharp composite plus a WebP encode. What makes that safe is not this limiter but
// the closed parameter space behind it — 11 languages × 4 styles × 10 slider stops, all
// cached — so a caller can force at most a few hundred renders in the lifetime of the
// process and gets memory hits thereafter. This bounds the rate at which they can walk
// that space from cold; the ceiling is loose because the real UI fires one request per
// slider settle, and a user sweeping the slider is the intended behaviour, not abuse.
// Imported directly by routes/staging.js — same rule as validateImageLimiter, an omitted
// dep must never leave an unauthenticated endpoint unlimited. Tests override it via the
// router's `disclosurePreviewLimiter` dep.
export const disclosurePreviewLimiter = rejectingLimiter({
  name: 'disclosure_preview',
  windowMs: 5 * 60 * 1000,
  limit: Number(process.env.RL_DISCLOSURE_PREVIEW || 120),
  message: { error: 'Too many preview requests. Please wait a moment and try again.' },
});

// Disclosure stamping of a finished client composite (POST /api/stamp-image). Basic Mask
// builds its result in the browser and downloads it from there, so this is the ONE place
// the server ever sees those pixels — which makes it the second endpoint (after
// /api/masking-studio/save) that accepts megabytes of client-supplied image with NO model
// call in between, and therefore without the cost-and-latency brake a generation provides.
// Each accepted request is a sharp decode, a composite and a PNG encode over a full-size
// render, so the work is real even though nothing is stored.
//
// Sized for the interaction it serves: one request per finished image the user chooses to
// download, and every one of those cost at least one /api/mask-edit call first, which spends
// genLimiter's budget. 30 per 15 minutes leaves someone downloading a batch of edits plenty
// of room while bounding how much CPU a stolen session can burn on free composites.
// Imported directly by routes/staging.js — same rule as validateImageLimiter and
// galleryImportLimiter: an omitted dep must never leave a paid, CPU-spending endpoint
// unlimited. Tests override it via the router's `stampImageLimiter` dep.
export const stampImageLimiter = rejectingLimiter({
  name: 'stamp_image',
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.RL_STAMP_IMAGE || 30),
  message: { error: 'Too many label requests. Please wait a few minutes and try again.' },
});

// Server-side resize + re-encode of a finished staging result for download (POST
// /api/download-result). Unlike stampImageLimiter this is NOT a paid-generation or
// Pro-only surface — it backs the free homepage staging tool's own download button, which
// used to do this resize entirely client-side on <canvas> and cost the server nothing.
// The ceiling exists only because the route now spends real CPU (a sharp decode, resize
// and JPEG re-encode per click) with no model call in between — cheaper work than
// stampImageLimiter's PNG composite, so it gets its own generous budget rather than
// sharing that limiter's "paid generation" framing. Imported directly by
// routes/staging.js, same rule as every other CPU-spending endpoint here: an omitted dep
// must never leave it unlimited. Tests override it via the router's
// `downloadResultLimiter` dep.
export const downloadResultLimiter = rejectingLimiter({
  name: 'download_result',
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.RL_DOWNLOAD_RESULT || 60),
  message: { error: 'Too many downloads. Please wait a moment and try again.' },
});
...
// These four differ from everything above in WHO they are keyed on. Every other
// limiter here buckets by IP, which is the only identity an anonymous caller has. An
// API caller has a better one: the key id. Keying the render budget on the key means
// one customer's busy afternoon cannot spend another customer's budget just because
// both sit behind the same NAT or the same cloud provider's egress range — which, for
// a server-to-server API, is the normal case rather than the exception.

/**
 * Bucket an authenticated API request by its key, falling back to IP.
 *
 * The fallback matters for ordering: this limiter runs BEFORE requireApiKey (so a
 * flood of unauthenticated junk is refused before it reaches a database lookup), and
 * at that point `req.apiKey` is not set yet. So in practice the IP branch bounds the
 * unauthenticated noise and the key branch bounds real customers.
 * @param {any} req - The request.
 * @returns {string} The bucket key.
 */
function apiKeyOrIp(req) {
  const id = req && req.apiKey && req.apiKey.id;
  return id ? 'k:' + id : 'i:' + (req?.ip || 'unknown');
}

// The render budget. Sized well above any sane integration's steady state — the real
// brake on this endpoint is the credit balance, which is a ceiling a rate limiter
// cannot express, and the concurrency gate (lib/http/api-concurrency.js), which is the
// one that actually protects the box.
export const apiRenderLimiter = rejectingLimiter({
  name: 'api_render',
  windowMs: 5 * 60 * 1000,
  limit: Number(process.env.RL_API_RENDER || 60),
  message: { error: 'Too many requests. Please slow down and try again shortly.', code: 'RATE_LIMITED' },
  keyGenerator: apiKeyOrIp,
});

// WRONG keys only, per IP — applied inside lib/http/api-key-auth.js, exactly as
// endpointKeyLimiter is applied inside http-guards.js and for the same reason: that is
// the only place that knows an attempt was actually rejected. A VALID key never touches
// this bucket, so a busy integration cannot lock itself out by being busy.
export const apiKeyRejectLimiter = rejectingLimiter({
  name: 'api_key_reject',
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.RL_API_KEY_REJECT || 20),
  message: { error: 'Too many attempts. Please wait a few minutes and try again.' },
});

// Key management (create/revoke/rename). Session-authenticated, so this is a cost
// backstop on the mint path rather than an access control.
export const apiKeyManageLimiter = rejectingLimiter({
  name: 'api_key_manage',
  windowMs: 15 * 60 * 1000,
  limit: Number(process.env.RL_API_KEY_MANAGE || 30),
  message: { error: 'Too many requests. Please wait a few minutes and try again.' },
});

// Credit-pack checkout. Mirrors checkoutLimiter's framing — each accepted request
// creates a real Stripe Checkout Session, and an abandoned session is litter in the
// Stripe dashboard rather than a charge, so the ceiling is about noise, not fraud.
export const creditCheckoutLimiter = rejectingLimiter({
  name: 'api_credit_checkout',
  windowMs: 60 * 60 * 1000,
  limit: Number(process.env.RL_API_CREDIT_CHECKOUT || 10),
  message: { error: 'Too many checkout attempts. Please wait a while and try again.' },
});
