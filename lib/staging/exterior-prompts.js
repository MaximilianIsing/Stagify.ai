...
// IMAGE_FRAMING_PRESERVATION_RULES is gone. It was one block of framing rules shared by both
// studios, and neither of them should have been using it: it granted the camera a crop
// exception that BOTH preservation blocks then denied (and, speaking last, both won), and its
// wording was interior-only — "ceiling line", "floor line", "placing new furniture" — which on
// an exterior photo asked for the staging this tool exists to refuse. Each path now states its
// own framing rules inside its own preservation block, where they have authority and nothing
// below can contradict them. See EXTERIOR_PRESERVATION_RULES here and
// INTERIOR_PRESERVATION_RULES in ./preservation-rules.js.
...
export const CLEANUP_CLAUSES = {
  ...
  removeLeaves:
    'Clear fallen leaves and loose garden debris from the scene: drifts of leaves on the '
    + 'driveway, paths, steps, porch, lawn, beds and gutters, together with twigs, blown '
    + 'petals, seed pods, grass clippings and windfall fruit. Reconstruct each surface as it '
    + 'plausibly continues from the parts of it already visible in the photograph, matching '
    + 'their material, colour, texture, wear and condition. KEEP THE SEASON — this is the '
    + 'same day with the ground swept, not a summer photograph: trees and shrubs keep the '
    + 'leaves they still have, in the colour they have, bare branches stay bare, dormant or '
    + 'brown grass stays dormant and brown, and nothing is greened up, replanted, or brought '
    + 'into leaf or flower.',
};
 ...
 * still worth having. Tier 2 also yields to any rule stated above it — this block speaks
 * last and so wins every argument, which is right for the property and wrong for finish.
 *
 * FRAMING LIVES HERE, and did not always. It used to be a separate `CRITICAL — IMAGE
 * FRAMING` section built from a constant shared with interior staging, and that sharing was
 * a bug in two directions:
 *
 *   1. It granted the camera an exception — "move it ONLY if the user explicitly asked for a
 *      closer or different crop" — that this block denies outright. Speaking last, this
 *      block won, so a crop request was refused by a rule nobody had decided on.
 *   2. It was written for INTERIORS and never re-worded. On a photograph of a building it
 *      told the model to keep "the entire ceiling line, floor line, and all walls" in frame,
 *      and to "fit every staging change inside the existing frame, scaling and placing NEW
 *      FURNITURE so the entire original ROOM stays visible" — while the bullet above forbids
 *      adding furniture or staging of any kind. The exterior prompt was asking for the exact
 *      thing the tool exists to prevent.
 *
 * The camera is tier 1 (absolute), matching the interior decision and for the same reason:
 * exterior-handler.js hands the pristine upload to renderPersistence as the gallery's
 * `before` image, and a re-cropped render would no longer line up with it. It is stated in
 * the FRAMING section and NOWHERE else — the footprint bullet above deliberately stops at
 * "storey count" rather than repeating it, because two bullets owning the camera is the
...
- Do not change the building's footprint, height or storey count.
...
- Where you were asked ABOVE to remove something — a vehicle, clutter, a person, snow, standing water, fallen leaves — you may rebuild only what that thing was hiding, continuing the surrounding material, colour, texture, wear and condition exactly as they appear elsewhere in the photograph. Uncovering a surface is never permission to improve it: what comes back must be the same driveway, path, step or lawn on the same day, not a cleaner, newer, greener or better-kept one. If you cannot tell what was underneath, match the nearest visible stretch of the same surface.
...
- Hold the camera exactly where it is: same position, angle, lens, field of view, zoom and framing. No request may re-crop, zoom, or re-frame the photograph.
- Output the image at the EXACT same aspect ratio, orientation and canvas dimensions as the input photo, with the geometry undistorted — no stretching, squashing, letterboxing or padding, and no leaning or corrected verticals beyond what was asked for above.
- Keep the FULL scene from the input in frame — every edge and corner, the entire roofline and chimneys, the ground line, and the full width of the property and its boundary as they appear in the original must stay visible.
 ...
 * @property {boolean} [removeLeaves] - Clear fallen leaves and garden debris, keeping the season otherwise intact.
...
export const CLEANUP_LABELS = {
  ...
  removeLeaves: { phrase: 'fallen leaves cleared', title: 'leaves cleared' },
};
...
// EXTERIOR_REVIEW_PROMPT used to live here: a 38-line QA rubric for exterior renders. It was
// DEAD — exterior-handler.js passed it as `reviewBasePrompt` while also setting
// `skipQualityReview: true`, and the skip short-circuits in staging-generation.js before the
// rubric is ever read. It never reached a model.
//
// The exterior quality gate is off on purpose (re-rolling a relight produces a different sky,
// not a better one, at three times the cost), so if it is ever switched back on, write the
// rubric then against what the edit actually produces — the old one had already drifted out of
// sync with the shared reply format it claimed to match.

/**
 * The Exterior Studio's upload gatekeeper.
 *
 * WHY THIS EXISTS RATHER THAN REUSING STAGEABLE_IMAGE_CHECK_PROMPT
 * The interior gate rejects `5 = a car or other vehicle` and `7 = the outside of a
 * building`, and its accept-list covers outdoor living areas photographed from inside
 * them but no facade or street view. Pointed at this feature's canonical input — a front
 * elevation with a car on the drive — it would reject the exact photo the tool exists to
 * fix, as VEHICLE or as EXTERIOR.
 *
 ...
 *
 * `7` (EXTERIOR) is the same hazard, one degree worse. The interior gate now carries a
 * digit for "the outside of a building" precisely so it can hand those uploads to THIS
 * tool — which makes a front elevation the one verdict this gate must never honour. The
 * prompt below does not offer 7, but a grader answering from the interior taxonomy would
 * refuse a facade at the tool built to enhance facades, and the browser would then offer
 * the user a link to the page they are already standing on.
 * @type {ReadonlySet<string>}
 */
export const EXTERIOR_IGNORED_CODES = new Set(['5', '7']);
