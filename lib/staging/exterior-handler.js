...
import {
  buildExteriorPrompt, describeExteriorRequest, describeExteriorQualifier, EXTERIOR_REVIEW_PROMPT,
  ...
  async function handleExteriorMultipart(req, res, user) {
    ...
    const image = await processStaging(
      mainFile.buffer,
      {
        ...
        // ...which also means the interior QA rubric would be grading a facade against
        // "warped furniture" and "extra legs". Swap it — it is still what a retry would
        // judge against if the gate is ever switched back on here.
        reviewBasePrompt: EXTERIOR_REVIEW_PROMPT,
