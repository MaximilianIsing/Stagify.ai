...
export function createSegmentHandler(deps) {
  ...
  return async (req, res) => {
    try {
      ...
      // Was `image.includes(',')`, which let 'a,b' through to the decode below and
      // on into sharp. Same shape check as validate-image and mask-edit now.
      ...
      const rawBuffer = decodeImageDataUrl(image);
      let imageBuffer;
      try {
