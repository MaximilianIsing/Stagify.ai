...
import { downscaleImage, enforceAspectRatio, padBufferToAspectRatio, orientedDimensions } from '../image/image-primitives.js';
 ...
 * @param {{ genAI: { getGenerativeModel: (options: any) => any }, DEBUG_MODE: boolean, runQualityRetry: Function, reviewImageQuality: Function, QUALITY_MAX_ATTEMPTS: number, logPromptToFile: Function }} deps - Injected Gemini client, debug flag, the options-object generateWithQualityRetry (runQualityRetry), the quality reviewer, the attempt cap, and the CSV prompt logger.
...
export function createStagingGeneration(deps) {
   ...
   * @param {Function | null} [reviewFn=null] - Quality reviewer; defaults to the injected reviewImageQuality.
...
Composition: frame the full scene naturally, keeping ceilings, floors, walls, and the key subject matter completely in view (use a tight crop or close-up ONLY if the user explicitly requested one).`;
      ...
      return await generateWithQualityRetry(async (attempt, feedback) => {
  ...
  async function processStaging(imageBuffer, stagingParams, req, furnitureImageBuffer = null, geminiModel = 'gemini-2.5-flash-image') {
    try {
      ...
      // Lock the result back to the source aspect ratio: enforceAspectRatio stretches
      // only the output's height (fit: 'fill', no crop or padding) to hit the source
      // ratio when Gemini's drift falls in the 1–8% correction band; drift under 1% or
      // over 8% is left untouched.
      if (srcDims) {
