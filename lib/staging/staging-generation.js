...
import { downscaleImage, padBufferToAspectRatio, orientedDimensions, upscaleForDelivery, resolveAspectRatioPin, cropToAspectRatio } from '../image/image-primitives.js';
...
Composition: frame the full scene naturally, keeping ceilings, floors, walls, and the key subject matter completely in view (use a tight crop or close-up ONLY if the user explicitly requested one).`;
  ...
  async function processStaging(imageBuffer, stagingParams, req, furnitureImageBuffer = null, geminiModel = 'gemini-2.5-flash-image') {
    ...
    // Whether the QA reviewer said the DELIVERED image changed the room's architecture.
    // Recorded per render so the rate is a number on the dashboard rather than a hunch —
    // this defect class was invisible for exactly as long as nothing counted it. Stays
    // `null` (logged as unknown) when the gate is off or the reviewer never saw a source.
    /** @type {boolean | null} */
    let architectureDrift = null;
    // The base seed this render used, so the row is enough to re-run it. Assigned below,
    // once we know the request is actually going to reach the model.
    /** @type {number | null} */
    let seedBase = null;
    const logOutcome = (status, errorCode) => {
      ...
      logPromptToFile(
        ...
        req,
        {
          ...
          architectureDrift,
          seed: seedBase,
          // "n/N" when this render is one of several variations of one request, blank
          // otherwise. Pairs with architectureDrift above: the two columns together are
          // what make "do the extra variations drift more?" a query rather than a hunch.
          variation: stagingParams.variation ?? null,
        },
      );
    };
    ...
    try {
      ...
      //
      // resolveAspectRatioPin, not nearestGeminiAspectRatio: the pin is now SKIPPED when no
      // supported bucket actually fits the upload. Demanding a reshape the model can only
      // satisfy by inventing or discarding frame edges was buying round-trip stability at
      // the cost of the architecture at those edges.
      ...
      // One base seed per render, shifted per attempt below.
      //
      // Nothing depends on the VALUE — the point is that it is written to the render log, so
      // a "this render bricked up my window" report can actually be re-run. Without it every
      // reproduction attempt was a fresh roll of the dice, which is the main reason this
      // whole defect class was argued about rather than measured. Same int32 clamp as
      // mask-edit: Gemini rejects seeds outside [0, 2^31-1], and the per-attempt +1 must not
      // overflow either.
      seedBase = randomInt(0, 0x7ffffff0);
      // Built per attempt rather than once, so the seed can shift when the quality gate
      // retries — an identical seed would just re-court the output that was rejected. This
      // is the shape mask-edit already uses.
      const modelForAttempt = (attempt) => genAI.getGenerativeModel({
        model: geminiModel,
        ...
        generationConfig: {
          seed: (seedBase + attempt - 1) % 0x7fffffff,
          ...(arPin ? { imageConfig: { aspectRatio: arPin.label } } : {}),
        },
      });
      ...
      // The exact bytes the image model was conditioned on — not `imageBuffer`, which may
      // still carry an EXIF rotation and a different resolution. The reviewer has to compare
      // like with like, or it reports "the camera moved" on every rotated phone photo.
      const sourceReviewUrl = `data:image/jpeg;base64,${base64Image}`;

      // Records each verdict's architecture call as it happens. The retry loop returns only
      // the winning URL, not the verdict that chose it, so the value has to be captured on
      // the way past. Last write wins, which is the ACCEPTED attempt in every case that
      // matters: the loop returns immediately on a perfect verdict, and when nothing is
      // perfect the drift-capped score above makes a drifted attempt lose to any clean one.
      /**
       * @param {(url: string) => Promise<import('../types/image.js').ImageReviewResult>} reviewFn
       * @returns {(url: string) => Promise<import('../types/image.js').ImageReviewResult>}
       */
      const recordDrift = (reviewFn) => async (url) => {
        const verdict = await reviewFn(url);
        if (typeof verdict?.architectureDrift === 'boolean') architectureDrift = verdict.architectureDrift;
        return verdict;
      };

      const reviewer = stagingParams.skipQualityReview
        ? async () => ({ perfect: true, score: 100, reason: 'quality gate disabled for this path' })
        : recordDrift((/** @type {string} */ url) => reviewImageQuality(url, {
          ...
          // THE SOURCE PHOTO. Without this the reviewer sees only the render and is
          // structurally unable to notice that a window became a wall — it has nothing to
          // compare against, so the drift scores PERFECT and ships. This one argument is
          // what turns the existing retry loop from blind into targeted; it costs nothing,
          // because the review request already fires on every attempt either way.
          sourceDataUrl: sourceReviewUrl,
          // Selects WHICH architecture clause the reviewer is given, and only that. Bathroom
          // staging is allowed to install sanitaryware, so a reviewer handed the default
          // clause reports every correct bathroom render as architecture drift — which sets
          // `perfect` false, caps the score at 20, and therefore burns the whole retry budget
          // on every bathroom render while logging a drift row that is not true. See
          // BATHROOM_ARCHITECTURE_REVIEW_CLAUSE in ./preservation-rules.js.
          roomType: stagingParams.roomType,
      ...
      const resultDataUrl = await generateWithQualityRetry(async (attempt, feedback) => {
        const attemptPrompt = feedback
          ...
          : prompt;
        const result = await modelForAttempt(attempt).generateContent(attemptPrompt);
        const response = await result.response;
      ...
      // ignores the pin: it centre-crops back (no stretch, no distortion) and no-ops when
      // the output is already close enough.
      //
      // It measures against the SOURCE ratio, not the pinned bucket. Those are within 2% of
      // each other whenever a pin was applied, so honored buckets are still left alone — but
      // when no bucket fit and nothing was pinned, this is now the only thing standing
      // between a drifted output and delivery. Judging against the pin meant the check
      // simply did not run in exactly the case that needed it.
      let finalDataUrl = resultDataUrl;
      const outputTargetRatio = arPin ? arPin.sourceRatio : (srcDims ? srcDims.width / srcDims.height : null);
      if (outputTargetRatio) {
        const m = /^data:image\/\w+;base64,(.+)$/.exec(resultDataUrl);
        if (m) {
          const cropped = await cropToAspectRatio(Buffer.from(m[1], 'base64'), outputTargetRatio);
      ...
      if (stagingParams.labelVirtuallyStaged) {
        finalDataUrl = await stampVirtuallyStaged(finalDataUrl, {
          lang: stagingParams.stampLang,
          style: stagingParams.stampStyle,
          scale: stagingParams.stampScale,
        });
      }
