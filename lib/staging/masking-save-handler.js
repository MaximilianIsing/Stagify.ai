...
// unusual below.
//
// TWO OUTCOMES, AND THE CHOICE IS NOT THE CLIENT'S TO MAKE
//   - REPLACE, when the studio was opened on a render the owner already has. The entry
//     keeps its id, its name, its position in the gallery and its share link, and only the
//     pixels change. The client asks for this by sending `renderId`, but the store decides:
//     `replaceResult` has ownership and liveness in its WHERE, so a guessed or stale id
//     simply fails to match.
//   - INSERT, for everything else — a photo dragged in from disk, an id that is not the
//     caller's, or a render still finishing its first upload. Falling back rather than
//     erroring is deliberate: the user pressed a button that means "keep this", and every
//     reason a replace can fail is a reason an insert is still correct.
//
...
export function createMaskingSaveHandler(deps) {
  ...
  async function handleMaskingSave(req, res, user) {
    const { after, before, renderId, areas, prompts, sourceName } = req.body ?? {};
    ...
    const afterBuffer = decodeImageDataUrl(after);
    if (afterBuffer.length > MAX_IMAGE_BYTES) {
    ...
    // ---- Replace, when the studio was opened on one of this owner's renders -----------
    if (typeof renderId === 'string' && renderId) {
      try {
        const replaced = await renderPersistence.replaceInBackground({
          renderId,
          user,
          native: afterBuffer,
        });
        if (replaced) {
          // No `gallery` payload: nothing was created and nothing was evicted, so there is
          // nothing for the client's eviction notice to say.
          return res.json({ success: true, replaced: true, gallery: null });
        }
        logger.info(`[gallery] masking save could not replace ${renderId}; inserting instead`);
      } catch (error) {
        logger.error('[gallery] masking replace failed; inserting instead:', error);
      }
    }
