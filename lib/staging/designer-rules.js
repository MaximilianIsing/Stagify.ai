...
// The Designer used to be told to write framing and architecture-preservation prose into
// EVERY additionalPrompt. It no longer is, and that instruction is not coming back:
//
//   • It was a THIRD copy of a rule the system already appends. `generatePrompt()` ends with
//     INTERIOR_PRESERVATION_RULES, which states preservation and framing once, last, with
//     explicit authority over everything above it. A model-improvised paraphrase of the same
//     rule, whose wording varies per turn, adds nothing and can only be weaker.
//   • It carried the crop carve-out — "a closer crop is allowed ONLY when the user explicitly
//     asked for one" — which the lock flatly denies. The routing model was being told to
//     write a permission the image model would then be told to ignore.
//   • The CAD path does not need it either: cad-handling.js appends its own framing line to
//     the top-down builder, and the eye-level builder deliberately has none (its input is a
//     2D plan it is not reproducing, so preserving the input's frame is exactly wrong).
//
// What remains is the one thing only the Designer knows: which entries are eye-level CAD, so
// the system does not pin a blueprint's shape onto a photograph.
export const AI_DESIGNER_IMAGE_FRAMING_RULES =
  '\n\nIMAGE FRAMING:' +
  '\n- Do NOT write framing, aspect-ratio, cropping or architecture-preservation instructions into any additionalPrompt. The system appends those rules itself, in stronger and more consistent wording than you can produce, and a second paraphrase only competes with them. Describe WHAT THE USER ASKED FOR and nothing else.' +
  '\n- In particular, never tell the image model that a closer crop or a different framing is allowed. It is not, on any staging request, however the user phrased it.';
