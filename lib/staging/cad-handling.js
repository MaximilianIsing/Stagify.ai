// CAD floor-plan → top-down 3D render, via the SHARED Gemini client.
//
 ...
 * its own.
 * @param {{ genAI: any }} deps - The shared GoogleGenerativeAI client (loosely typed; null when no key is configured).
 * @returns {{ blueprintTo3D: (blueprintImage: string|Buffer, mimeType?: string|null, furnitureImages?: import('../types/image.js').FurnitureImageDescriptor[], additionalPrompt?: string|null) => Promise<Buffer> }} The CAD handling API.
 */
export function createCadHandling({ genAI }) {
  return {
    blueprintTo3D: (blueprintImage, mimeType = null, furnitureImages = [], additionalPrompt = null) =>
      renderBlueprintTo3D(genAI, blueprintImage, mimeType, furnitureImages, additionalPrompt),
  };
}
...
/**
 * Converts a top-down blueprint/floor-plan into a top-down 3D render via Gemini.
 ...
 * createCadHandling() binds `genAI` and returns it as `blueprintTo3D`.
 ...
 * @param {string|null} [mimeType] - MIME type of the blueprint; auto-detected from the data URL / defaults to image/png when null.
 * @param {import('../types/image.js').FurnitureImageDescriptor[]} [furnitureImages] - Optional furniture reference images to place into the render (defaults to []).
 * @param {string|null} [additionalPrompt] - Optional extra user instructions (theme/style/requirements) folded into the prompt.
 * @returns {Promise<Buffer>} Buffer of the generated top-down 3D render (PNG bytes).
 */
async function renderBlueprintTo3D(genAI, blueprintImage, mimeType = null, furnitureImages = [], additionalPrompt = null) {
  if (DEBUG_MODE) {
    logger.debug("=== BLUEPRINT TO 3D RENDER ===\n");
  }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  (async () => {
    ...
    if (argv.length === 0) {
      if (DEBUG_MODE) {
        logger.debug("Usage: node cad-handling.js <blueprintImage> [outputImage]");
        logger.debug("");
        logger.debug("If only blueprint image is provided, output will be auto-generated as <name>-render.png");
        logger.debug("");
        logger.debug("Examples:");
        logger.debug("  node cad-handling.js Room1.png");
        logger.debug("  node cad-handling.js Room1.png output.png");
      }
      process.exit(1);
    }
