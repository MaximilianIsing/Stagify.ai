 ...
 * Reads the Google AI key from GOOGLE_AI_API_KEY/GEMINI_API_KEY or key.txt (throws
 * if none is found), caches the client per API key, and delegates image extraction
 * to parseGeminiResponse (which throws on a text-only or unexpected response shape).
...
async function blueprintTo3D(blueprintImage, mimeType = null, furnitureImages = [], additionalPrompt = null) {
  ...
  if (cachedGenAI && cachedApiKey === apiKey) {
    genAI = cachedGenAI;
    if (DEBUG_MODE) {
    logger.debug("Reusing cached Gemini client");
    }
  } else {
    if (DEBUG_MODE) {
...
export { blueprintTo3D, getMimeType, extractBase64, parseGeminiResponse };
