 ...
 * @param {{ genAI: { getGenerativeModel: (options: any) => any } | null, DEBUG_MODE: boolean, authStore: any, toPublicAuthUser: ReturnType<typeof import('../services/auth-helpers.js').createAuthHelpers>['toPublicAuthUser'], enterpriseDomainForUser: ReturnType<typeof import('../services/auth-helpers.js').createAuthHelpers>['enterpriseDomainForUser'], reportEnterpriseUsage: ReturnType<typeof import('../services/auth-helpers.js').createAuthHelpers>['reportEnterpriseUsage'], roomIsAlreadyEmpty: (imageBuffer: Buffer) => Promise<boolean>, eraseFurniture: ReturnType<typeof import('../image/erase.js').createErase>['eraseFurniture'], processStaging: ReturnType<typeof import('./staging-generation.js').createStagingGeneration>['processStaging'] }} deps - Injected Gemini client, debug flag, auth store + user shaper, enterprise-usage metering, the empty-room check, the eraser, and processStaging (from the createStagingGeneration factory — instantiate this AFTER it).
...
export function createVirtualStagingHandler(deps) {
  ...
  async function handleVirtualStagingMultipart(req, res, meta) {
    ...
    if (meta.recordUsage && user) {
      ...
      } else if (user.plan === 'pro' && typeof authStore.recordStagingActivity === 'function') {
        // Trial/paid activity signal for the trial-email sweep + the "you staged N
        // rooms" recap. Guarded so a mocked authStore in tests need not define it.
