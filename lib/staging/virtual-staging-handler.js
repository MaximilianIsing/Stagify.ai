...
export function createVirtualStagingHandler(deps) {
  ...
  async function handleVirtualStagingMultipart(req, res, meta) {
    ...
    // "Label as virtually staged" burns the disclosure into the pixels. Read WITHOUT any
    // `isPro &&` — the omission is deliberate and must stay: this is an MLS / NAR compliance
    // control, not a feature to upsell, and paywalling it would push free users to publish
    // undisclosed staged photos.
    //
    // The coercion, the locale check and the style allow-list all live in readStampRequest
    // now: /api/masking-studio/save and /api/stamp-image ask the same question of the same
    // browser controls, and three copies of "which badges are real" is three chances for one
    // surface to accept a value another rejects.
    const {
      enabled: labelBool,
      lang: stampLangSafe,
      style: stampStyleSafe,
      scale: stampScaleSafe,
    } = readStampRequest({ labelVirtuallyStaged, stampLang, stampStyle, stampScale });
    ...
    const stagingParamsBase = {
      ...
      labelVirtuallyStaged: labelBool,
      stampLang: stampLangSafe,
      stampStyle: stampStyleSafe,
      stampScale: stampScaleSafe,
      // Off for every browser surface, on for the public API — see VirtualStagingMeta.
      // Read in staging-generation.js, which swaps the reviewer for a synthetic pass;
      // maxAttempts is deliberately untouched there, so a transient provider error is
      // still retried while a merely-imperfect result is not.
      skipQualityReview: !!meta.skipQualityReview,
    };
    ...
    // with the same prompt — so they run CONCURRENTLY. Serially, the paid tier was the
    ...
    //
    // DIVERSITY COMES FROM THE SEED, NOT FROM THE PROMPT, and that is deliberate.
    // Variations 2..N used to carry an appended "(Subtle variation n of N: ...)"
    // parenthetical inside `additionalPrompt`. It did three things wrong:
    //   1. It defeated the empty-custom fallback. generatePrompt() promotes
    //      additionalPrompt to BE the base prompt on the 'custom' style and falls back to
    //      'standard' only when the box is empty (./prompts.js). Appending made the box
    //      non-empty, so a Custom-with-nothing-typed request lost its whole staging brief
    //      on variations 2..N — the exact "model improvises the room, architecture
    //      included" case that fallback exists to prevent — while variation 1 kept it.
    //   2. It paraphrased the architecture lock inside the user's free-text box, the one
    //      place ./prompts.js tells every caller NOT to put one: the authoritative block
    //      is appended last with override authority, and it states the rule as a COUNT of
    //      windows/doors precisely because an abstract restatement is not checkable.
    //   3. It asked for a different furniture ARRANGEMENT with no spatial constraint, and
    //      only on variations 2..N — so the extra images drifted on walls and openings
    //      more than the first one did, which is what the feature was reported for.
    // Each processStaging call already rolls its own independent seedBase, so N identical
    // prompts still yield N different rooms. Do not reintroduce a per-variation prompt;
    // the drift rate per variation is now a column in prompt_logs.csv, so reach for that
    // before assuming the images are too alike.
    //
    ...
    const staged = await Promise.all(
      Array.from({ length: variationCount }, (_, v) => {
        const stagingParams = {
          ...stageBaseParams,
          // "n/N" — which variation of how many, carried into this render's
          // prompt_logs.csv row. One column answers both questions the row can be asked:
          // whether LATER variations drift more, and whether multi-variation requests
          // drift more than single ones. Only this handler sets it; every other
          // processStaging caller leaves it blank.
          variation: `${v + 1}/${variationCount}`,
    ...
    try {
      if (renderPersistence?.enabled() && user && natives.some(Boolean)) {
        const pending = renderPersistence.recordPending({
          ...
          // No `qualifier`: this is the one surface where the owner picked both halves of
          // the name themselves, so it keeps the derived `<Style> <Room type>` and storing
          // a copy of two columns would only give them something to go stale against. The
          // filename is what tells twenty "Standard Bedroom"s apart — a user who never
          // opens either dropdown gets the same name on every render they ever make.
          extra: { source: meta.sourceTag || 'interior', sourceName: mainFile.originalname },
          model: geminiModel,
        });
        if (pending) {
          // `tier` exists so the client can say the right thing about an eviction. A free
          // account is told its plan keeps only the most recent; a Stagify+ account is
          // told nothing about the cap at all, because Stagify+ is sold as unlimited
          // STAGING (which it is) and the gallery ceiling is not advertised. It is not a
          // secret either — it is in lib/data/staged-renders.js — just not something to
          // surface unprompted.
          gallery = { ids: pending.entries.map((e) => e.id), evicted: pending.evicted, tier: isPro ? 'pro' : 'free' };
