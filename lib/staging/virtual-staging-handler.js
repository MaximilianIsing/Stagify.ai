...
import { STAMP_STYLE_NAMES, DEFAULT_STAMP_STYLE, clampStampScale } from '../image/stamp-disclosure.js';
...
export function createVirtualStagingHandler(deps) {
  ...
  async function handleVirtualStagingMultipart(req, res, meta) {
    ...
    // "Label as virtually staged" burns the disclosure into the pixels. Coerced exactly
    // like removeBool above but WITHOUT the `isPro &&` — the omission is deliberate and
    // must stay: this is an MLS / NAR compliance control, not a feature to upsell, and
    // paywalling it would push free users to publish undisclosed staged photos.
    const labelBool = !!(
      labelVirtuallyStaged === true ||
      labelVirtuallyStaged === 'true' ||
      labelVirtuallyStaged === 'on');

    // The language decides which pre-rendered badge master gets composited. It comes from
    // the browser, so validate against the real locale set rather than trusting it into a
    // filename; anything unrecognized falls back to English inside the stamp module.
    const stampLangSafe = ALL_LOCALES.some((l) => l.lang === stampLang) ? stampLang : 'english';

    // Look and size come from the same place and get the same treatment: an allow-list for
    // the style (it selects a code path, so an unknown value must never reach it) and a
    // clamp for the multiplier (a slider that posts 400 would otherwise ask sharp for a
    // badge wider than the photo). Both fall back rather than reject — see resolveStampStyle.
    const stampStyleSafe = STAMP_STYLE_NAMES.includes(String(stampStyle)) ? String(stampStyle) : DEFAULT_STAMP_STYLE;
    const stampScaleSafe = clampStampScale(stampScale);
    ...
    try {
      if (renderPersistence?.enabled() && user && natives.some(Boolean)) {
        const pending = renderPersistence.recordPending({
          ...
          extra: { source: 'interior', sourceName: mainFile.originalname },
          model: geminiModel,
        });
        if (pending) {
