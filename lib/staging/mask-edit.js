...
//         enterpriseDomainForUser, reportEnterpriseUsage, recordStagingActivity }
import sharp from 'sharp';
import { isImageDataUrl, decodeImageDataUrl } from './data-url.js';
import { sendError } from '../http/http-helpers.js';
import { reportError } from '../http/error-ref.js';
import { resolveChatModel } from '../config/model-config.js';
...
export function createMaskEditHandler(deps) {
  ...
  return async (req, res) => {
    try {
      ...
      const { image, mask, prompt, model, referenceImage, seed } = req.body;

      const trimmedPrompt = typeof prompt === 'string' ? prompt.trim() : '';
      if (!image || !mask || !trimmedPrompt) {
        return sendError(res, 400, 'Image, mask, and prompt are required');
      }
      // Validate the SHAPE, not just truthiness. Only presence was checked here, so
      // `{"image":"x","mask":"y","prompt":"z"}` reached Buffer.from(undefined,'base64')
      // below and threw a TypeError — answered as a 500 with an error ref and a Sentry
      // report, i.e. every malformed client payload became a fake production incident.
      // referenceImage is deliberately NOT gated here: it is optional, and the
      // existing path below degrades gracefully (warns, edits without it) rather
      // than failing the whole request. Only image and mask reach a bare decode.
      if (!isImageDataUrl(image) || !isImageDataUrl(mask)) {
        return sendError(res, 400, 'Image and mask must be base64 image data URLs');
      }
      ...
      const seedBase = Number.isInteger(seed)
        ...
        : null;
      // Every area gets the SAME quality-retry budget. This used to be trimmed to 2
      // for a fan-out of 3+ areas as a cost guard, which meant the Masking Studio got
      // quietly worse the more areas a user painted — a paid feature degrading exactly
      // when it is used hardest, with nothing in the UI saying so. Cost is bounded by
      // the per-IP generation limiter and by the fact that retries only fire on a
      // not-perfect verdict, so the ceiling is worst-case, not typical.

      // Allow-listed like every other client-supplied model. Nothing here calls
      // OpenAI with it (geminiModel below is pinned), but it lands in the CSV log
      // — so an unfiltered value would poison the model column of the analytics.
      ...
      // Convert base64 data URLs to buffers — safe now that the shape is validated.
      /** @type {Buffer} */
      let imageBuffer = decodeImageDataUrl(image);
      let maskBuffer = decodeImageDataUrl(mask);
      ...
      if (referenceImage && typeof referenceImage === 'string' && referenceImage.includes(',')) {
        ...
        try {
          ...
          enhancedPrompt += maskReferencePromptSuffix(loc);
          logger.info(
      ...
      // Log the mask edit request, keyed on the validated session account
      ...
      if (entDomain) {
        ...
        // The Masking Studio is Stagify+ only, so this IS trial usage — but it used
        // to write no activity timestamp at all, leaving a trial user who lived in
        // this tool classified as "never staged anything" by the lifecycle sweep.
        recordStagingActivity(proUser);
      }
      ...
      return sendError(res, 500, 'Failed to process masked edit', { ref: reportError('staging.mask-edit', error) });
    }
  };
}
