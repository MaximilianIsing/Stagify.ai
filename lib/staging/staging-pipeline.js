...
//
// A verdict marked `degraded` (reviewer disabled or throwing — see ImageReviewResult)
// still accepts the image, because failing closed would turn a reviewer outage into a
// user-facing outage. But it is reported through `onReviewDegraded` and logged at warn,
// so "shipped without QA" is recorded rather than silently indistinguishable from a
// flawless run. Deliberately NOT retried: re-rolling against a broken reviewer just
// multiplies cost without improving the odds.
 ...
 * @param {(attempt: number, feedback: import('../types/image.js').ImageReviewResult | null) => (Promise<string|null>|string|null)} generateOnce - Produces one image (a data-URL string) for the given 1-based attempt, or null to skip; may throw. `feedback` is the previous attempt's not-perfect QA verdict (null on the first attempt), so the generator can target the named defect.
 * @param {{
 *   label?: string,
 *   onImageProduced?: ((attempt: number) => void) | null,
 *   onReviewDegraded?: ((attempt: number, review: import('../types/image.js').ImageReviewResult) => void) | null,
 *   reviewFn?: (url: string) => Promise<import('../types/image.js').ImageReviewResult>,
 *   maxAttempts?: number,
 *   debug?: boolean,
 * }} [options] - Retry config; `reviewFn` and `maxAttempts` are typed optional because the `= {}` default allows omission, but both are validated as required at runtime (throws otherwise). `onReviewDegraded` fires when an image is accepted on an unreviewed (degraded) verdict.
...
export async function generateWithQualityRetry(generateOnce, {
  ...
  let lastError = null;
  let lastReview = null; // previous attempt's not-perfect verdict, fed back into generateOnce so it can target the defect
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    let url;
    try {
      url = await generateOnce(attempt, lastReview);
    } catch (error) {
      lastError = error;
    ...
    const review = await reviewFn(url);
    const { perfect, score } = review;
    // An unreviewed acceptance is NOT a pass. Report it before returning so the
    // caller can record that this image shipped without QA; warn unconditionally
    // (not behind `debug`) because a silent reviewer outage is the failure mode
    // this exists to expose.
    if (review && review.degraded) {
      logger.warn(`[Quality] ${label} attempt ${attempt}/${maxAttempts}: accepted WITHOUT review (${review.reason || 'reviewer unavailable'}).`);
      if (typeof onReviewDegraded === 'function') onReviewDegraded(attempt, review);
      return url;
    }
    if (debug) {
    ...
    lastReview = review; // carry the named defect into the next attempt's prompt
    if (debug && attempt < maxAttempts) {
