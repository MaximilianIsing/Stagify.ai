...
export function initializePromptCount() {
  try {
    ...
    if (fs.existsSync(logFile)) {
      ...
      if (DEBUG_MODE) {
        logger.debug('Prompt count successfully initialized from file:', promptCount);
      }
    } else {
      if (DEBUG_MODE) {
        logger.debug('No prompt log file found, starting with count 0');
      }
      promptCount = 0;
    }
  } catch (error) {
    logger.error('Error initializing prompt count:', error);
    promptCount = 0;
  }
}
...
export function initializeContactCount() {
  try {
    ...
    if (fs.existsSync(logFile)) {
      ...
      if (DEBUG_MODE) {
        logger.debug('Contact count successfully initialized from file:', contactCount);
      }
    } else {
      if (DEBUG_MODE) {
        logger.debug('No contact log file found, starting with count 0');
      }
      contactCount = 0;
    }
  } catch (error) {
    logger.error('Error initializing contact count:', error);
    contactCount = 0;
  }
}
