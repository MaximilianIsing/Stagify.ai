...
export function createTrialTracking({
  ...
  function listTrialCandidates() {
    // Narrowed in SQL rather than reading the whole users table. `plan` is a real
    // column and the first thing the filter below tests, so pushing it into the
    // query skips the extra_json JSON.parse for every non-pro row — the bulk of
    // them. Safe because rowToUser's only plan effect (applyGrantExpiry) is a
    // DOWNGRADE pro→free, never an upgrade: the rows dropped here could not have
    // survived the filter anyway. The JS `plan === 'pro'` test therefore stays,
    // and is what still excludes an expired comp grant whose row says 'pro'.
    return proUserRows()
      .map(rowToUser)
