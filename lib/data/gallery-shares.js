...
// staged render, and `showBefore` — which defaulted TRUE over there — now defaults off
// (see below).
//
// WHY THIS EXISTS
...
//
// `showBefore` IS A SETTING AGAIN, AND IT DEFAULTS OFF. This module used to argue that the
// share page must never show the source photograph, and the omission was structural in
// both this file and routes/share-public.js. That was too strong: the person on the other
// end of the link is usually the seller who TOOK the empty-room photo, or a buyer being
// shown what the room becomes, and for either of them the before/after pair is the point.
// So it is published — but only when the owner ticks the box for that render, never by
// default, and only on a strict `=== true`. A truthy string arriving from anywhere may not
// publish a customer's photograph.
//
// The default is the reason the old argument does not simply come back: a link the owner
// has not touched still shows the staged result alone, exactly as before.
//
...
import { ACTIVE_FOR_RENDERS_SQL } from './gallery-page-reads.js';
 ...
 *
 * `showBefore` is the one non-string, and it is compared with `===` rather than coerced.
 * Every other key here is text somebody typed; this one decides whether a photograph of
 * somebody's actual empty house goes out to whoever holds the link, so `"false"`, `0` and
 * `'off'` all have to mean off. Truthiness is the wrong test for a publish switch.
 *
 * NOTE it rebuilds the whole bag from `raw`, so a caller sending one key BLANKS the rest.
 * That is why the gallery UI PATCHes the entry's full settings rather than a delta.
 *
 * @param {unknown} raw @returns {{ headline: string, note: string, agentName: string,
 *   agentEmail: string, agentPhone: string, showBefore: boolean }}
 */
export function normalizeShareSettings(raw) {
  ...
  return {
    ...
    showBefore: bag.showBefore === true,
  };
}
...
export function createGalleryShares(baseDir) {
  ...
  const q = {
    ...
    // The same question for a PAGE of renders — see gallery-page-reads.js, which carries the
    // "first row per render reproduces LIMIT 1" reasoning and the revoked-never-reused rule.
    // In the steady state there is one live row per render anyway (mintOrReuse enforces it),
    // so the tiebreak that makes it exact almost never fires.
    activeForRenders: db.prepare(ACTIVE_FOR_RENDERS_SQL),
  ...
  /**
   * The live share for each of a page of renders, or nothing for the renders that have
   * none. One statement for the whole page — see the `activeForRenders` statement above.
   * @param {string[]} renderIds
   * @returns {Map<string, any>} renderId → the newest live row.
   */
  function liveByRender(renderIds) {
    const out = new Map();
    if (!renderIds?.length) return out;
    for (const row of /** @type {any[]} */ (q.activeForRenders.all(JSON.stringify(renderIds)))) {
      // FIRST wins, never last: the ORDER BY is newest-first within a render, so the first
      // row for an id is the one `activeForRender`'s LIMIT 1 would have returned.
      if (!out.has(row.render_id)) out.set(row.render_id, row);
    }
    return out;
  }
   ...
   * pays for one.
   *
   * `prefetched` lets a caller that has ALREADY looked the row up hand it over instead of
   * paying for the lookup again — which is the whole point of {@link liveByRender}. It
   * distinguishes `undefined` ("I did not look") from `null` ("I looked, there is none"),
   * and that distinction is the whole optimization: collapsing them would send every
   * render without a live share straight back to the per-row SELECT this exists to avoid,
   * and those are exactly the renders the batched read is trying to mint for.
   *
   * @param {{ renderId: string, userId: string, settings?: any, expiresAt?: number | null,
   *   now: number, prefetched?: any }} arg
   * @returns {{ token: string, share: any, created: boolean }}
   */
  function mintOrReuse({ renderId, userId, settings, expiresAt = null, now, prefetched }) {
    const live = /** @type {any} */ (
      prefetched === undefined ? q.activeForRender.get(renderId) : prefetched
    );
  ...
  return {
     ...
     *
     * ONE transaction for the whole page rather than one per render, and now ONE SELECT
     * for the whole page rather than one per render: the lookups are batched up front and
     * handed to mintOrReuse, so the steady state — every render already has a link — is a
     * single statement rather than PAGE_SIZE of them. The first listing after this shipped
     * writes up to PAGE_SIZE rows, which is the case worth not paying sixty commits for.
     *
    ...
    ensureForRenders({ renders, now = Date.now() }) {
      ...
      return db.transaction(() => {
        // Read inside the transaction, not before it, so the lookups and the mints that
        // follow see one consistent snapshot.
        const live = liveByRender(list.map((r) => r.id));
        const out = new Map();
        for (const render of list) {
          out.set(render.id, mintOrReuse({
            renderId: render.id,
            userId: render.user_id,
            now,
            // `?? null` matters: a render with no live share must arrive as null ("looked,
            // found nothing") rather than undefined, which would send mintOrReuse back to
            // its own SELECT for exactly the rows this read already covered.
            prefetched: live.get(render.id) ?? null,
          }).share);
        }
        return out;
      })();
     ...
     *
     * `expiresAt` is THREE-WAY on purpose: `undefined` leaves the expiry alone, `null`
     * clears it, a number sets it. It used to default to `null`, and the UPDATE writes
     * `expires_at = ?` unconditionally — so every settings edit silently un-expired the
     * link it was editing. Nothing sets an expiry today, which is precisely why that was
     * invisible: the trap was armed for whoever added the first one.
     *
     ...
     */
    updateSettings({ renderId, settings, expiresAt, now = Date.now() }) {
      const row = /** @type {any} */ (q.activeForRender.get(renderId));
      if (!row) return null;
      const nextExpiry = expiresAt === undefined ? (row.expires_at ?? null) : expiresAt;
      q.patchSettings.run(JSON.stringify(normalizeShareSettings(settings)), nextExpiry, row.token_hash);
      void now;
