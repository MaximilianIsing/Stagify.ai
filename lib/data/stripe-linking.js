...
import { isGrantActive } from './pro-grants.js';
...
export function createStripeLinking({
   ...
   *
   * The email fallback still has to exist, though the reason has narrowed. The
   * "Start free trial" button no longer reaches Stripe while signed out — it opens
   * the sign-up modal instead (public/scripts/stagify-plus.js), because a checkout
   * with no reference could not be mapped to anyone and those buyers paid for
   * nothing. What that does NOT cover: the Payment Link is a public URL that can be
   * opened directly or from an old bookmark, and a signed-in buyer can still type a
   * different address into Stripe's own form. Dropping the fallback would leave
   * those checkouts unmapped. What it must not do is *reassign* an entitlement.
   * Without the guard
  ...
  function activateProFromStripeCheckout({ userId, email, stripeCustomerId, stripeSubscriptionId }) {
    ...
    if (matchedBy === 'email') {
      ...
      // could later revoke. Only a LIVE entitlement blocks, and each half has to
      // be tested for liveness in its own way:
      //   - a cancelled subscription clears `stripeSubscriptionId`
      //     (applyStripeSubscriptionState), so its mere presence means live;
      //   - a comp grant does NOT clear `proGrantExpiresAt` when it lapses —
      //     applyGrantExpiry only flips `plan` back to 'free' and only
      //     revokeProGrant nulls the field, so the date stays on the record
      //     forever. Reading it as a boolean refused every later email-matched
      //     checkout by anyone who was ever comped: a real purchase, real money,
      //     no plan. isGrantActive() compares it against the clock instead.
      ...
      const holdsGrant = isGrantActive(user);
   ...
   * @returns {{ ok: true, userId: string, plan: string } | { ok: false, reason: 'bad_payload' | 'no_user' | 'stale_subscription' }} The outcome.
   */
  function applyStripeSubscriptionState(subscription) {
    ...
    if (proStatuses.includes(status)) {
      ...
      // A terminal status only ends access if it belongs to the subscription this
      // account CURRENTLY holds. Stripe gives no ordering guarantee and retries a
      // failed delivery for ~3 days, and the lookup above falls back to the customer
      // id — so a delayed 'deleted' for a REPLACED subscription lands on the account
      // that has since resubscribed. Writing plan:'free' there downgrades someone who
      // is still being charged, and (because the webhook gates the win-back email on
      // ok:true) emails them a "sorry to see you go" as well. The id guard below has
      // always been here; this is the same guard the plan write was missing.
      //
      // The tradeoff, deliberately accepted: this makes the stored subscription id
      // load-bearing. If it ever disagrees with Stripe, a REAL cancellation is
      // swallowed and the account keeps Stagify+ — the cheap direction to be wrong in,
      // and the reason below is logged (routes' webhook handler warns on !ok) so the
      // divergence is visible rather than silent.
      if (user.stripeSubscriptionId && user.stripeSubscriptionId !== subId) {
