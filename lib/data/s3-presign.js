...
//
// WHAT A GALLERY PAGE ACTUALLY PAYS, so the count is not mistaken for a cost.
// A full page is 60 entries x 3 roles = 180 URLs, and that number looks alarming next to a
// request budget until it is measured. 2026-08-04, this module, warm: 8.1 microseconds per
// URL, 1.46 ms for all 180. The signing-key cache below is most of why (see its own note) —
// without it this would be 720 HMACs instead of 4.
//
// The other half is bytes, not time: a signed URL is 369 characters, so 180 of them are
// ~65 KB of the response body, re-sent on every visit because the manifest is `no-store`.
// The only way to shrink that is to mint fewer URLs, which is a decision for the route that
// builds the manifest, not for this file — routes/gallery.js records why it does not.
//
