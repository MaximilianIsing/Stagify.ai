...
-- The admin console's site-wide reads (lib/analytics/api-usage.js) filter on
-- claimed_at ALONE. The composite above is left-anchored on user_id, so the planner
-- cannot use it for a query with no user predicate, and every operator page load
-- would scan the table. Cheap to carry: api_requests is append-mostly, and the two
-- settle paths update by primary key rather than by this column.
CREATE INDEX IF NOT EXISTS idx_api_requests_claimed ON api_requests (claimed_at);
`;
