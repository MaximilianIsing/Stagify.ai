...
import { createSessionRevocation } from './session-revocation.js';
...
// Throwaway salt for the miss paths that burn a hash to stay constant-cost —
// login()'s unknown-email / Google-only branches and startRegistration()'s
// already-taken branch. See the comments there. Never stored, never compared.
...
-- No index on users(email): the UNIQUE constraint on the column already builds one
-- (sqlite_autoindex_users_2), and EXPLAIN QUERY PLAN shows the planner choosing THAT
-- for an email lookup whether or not a second one exists — so idx_users_email was
-- never read from, and only cost another B-tree write on every insert and every
-- whole-row upsert. Dropped rather than merely un-created, because CREATE INDEX
-- IF NOT EXISTS leaves an already-built index in place on existing databases. The
-- DROP is idempotent, which is what lets it live in the schema DDL with no migration
-- runner (the absence of one is deliberate).
-- NB: no backticks in this string - it is a JS template literal.
...
export function createAuthStore(baseDir) {
  ...
  function startRegistration(email, password) {
    ...
    if (dup) {
      ...
      //
      // Skipping the pending row also skips the ~100ms scrypt the fresh path pays
      // below, so returning straight from here would answer by the clock exactly
      // what the identical response body refuses to answer. Burn one hash at the
      // current cost and discard it — same helper, same trick, as login()'s miss
      // paths. Everything else either branch does is microseconds beside it.
      burnPasswordHash(password, DUMMY_PASSWORD_SALT);
  ...
  function publicUser(user) {
    ...
    return {
      ...
      // Enterprise seats reach 'pro' through their email domain rather than a
      // subscription of their own — `enterpriseDomain` is stamped on by
      // enhanceUserWithEnterprise() (lib/services/auth-helpers.js), which every
      // user-facing response passes through via toPublicAuthUser(). The plan is
      // identical; this flag exists purely so the UI can show the Enterprise mark
      // in place of the Stagify+ one. A raw stored user has no such field, so the
      // internal callers below (login/register/recordFreeGeneration) report false.
      enterprise: !!user.enterpriseDomain,
    };
  }
  ...
  const { revokeUserSessions } = createSessionRevocation({ findUserById, dropSessionsForUser });
