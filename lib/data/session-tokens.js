 ...
 *
 * @param {{ prepare: (sql: string) => any, transaction: (fn: any) => any, exec: (sql: string) => any }} db
...
export function createTokenTables(db) {
  // Both tables are keyed by token, so the PRIMARY KEY already covers every lookup
  // that HAS a token. These cover the two that don't, and would otherwise scan the
  // whole table: `WHERE user_id = ?` (sign-out-everywhere, on every password reset)
  // and `WHERE exp < ?` (the pruners — sessions every 6h, resets on each reset flow).
  //
  // The CREATE TABLEs live in auth-store.js's SCHEMA, which runs before this factory;
  // the indexes live here, next to the statements that need them, because this module
  // owns those tables' queries. IF NOT EXISTS on every open means an existing database
  // picks them up on the next boot — there is no migration step to remember.
  db.exec(`
    CREATE INDEX IF NOT EXISTS idx_sessions_user ON sessions(user_id);
    CREATE INDEX IF NOT EXISTS idx_sessions_exp  ON sessions(exp);
    CREATE INDEX IF NOT EXISTS idx_resets_user   ON password_reset_tokens(user_id);
    CREATE INDEX IF NOT EXISTS idx_resets_exp    ON password_reset_tokens(exp);
  `);
