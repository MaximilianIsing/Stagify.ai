...
export function createEnterpriseStore(baseDir) {
  ...
  // The blast-radius chokepoint: every caller that upgrades a user to `pro`
  // because of their email domain comes through here (auth-helpers). The public
  // -provider guard is repeated here rather than trusted from the write path so
  // that a row which arrived some other way — a legacy enterprise-domains.json
  // import, importStore(), a hand-edited DB — still cannot grant Stagify+ to
  // every gmail.com account.
  ...
  function activateDomain({
    ...
    const d = String(domain).trim().toLowerCase();
    // Refuse to write the row at all. /api/enterprise/create-checkout rejects
    // these before Stripe, so reaching this means the domain came from
    // somewhere else (replayed webhook, hand-made session, dashboard-created
    // subscription) — exactly the case worth stopping.
    if (isPublicEmailDomain(d)) {
      return { ok: false, reason: 'public_email_domain', domain: d };
    }
  ...
  function applySubscriptionState(subscription) {
    ...
    if (activeStatuses.includes(status)) {
      ...
      // Same guard as the per-user path (lib/data/stripe-linking.js): a terminal
      // status only ends access if it belongs to the subscription this domain
      // CURRENTLY holds. Stripe does not order its deliveries and retries for ~3
      // days, and the lookup above falls back to the customer id, so a delayed
      // 'deleted' for a REPLACED subscription reaches a domain that has since
      // resubscribed. Here the blast radius is the whole company rather than one
      // account — every seat loses access at once — which is why this branch never
      // had even the id check its per-user twin does.
      if (entry.stripeSubscriptionId && entry.stripeSubscriptionId !== subId) {
