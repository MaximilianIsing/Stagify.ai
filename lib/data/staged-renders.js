 ...
 *
 * The owner's name, the style, the room type and the extra prompt, joined — so a term can
 * land in any of them and a multi-word query can straddle two. `custom_name` is included
 * as well as the other three rather than instead of them: a render named "412 Rosewood
 * Lane" is still a luxury bedroom, and searching "luxury" should find it.
 *
 ...
 *
 * Control characters go first and unconditionally: this string is written into the
 * gallery grid, the dialog heading and the card's aria-label, and a stray newline or
 * bidi override in any of those is a display bug at best. Slicing happens last so the
...
export function normalizeRenderName(raw) {
  ...
  // Spread, not slice: `.slice(80)` on a name ending in an emoji or any other astral
  // character cuts it between surrogates and stores half a code point.
  return [...cleaned].slice(0, MAX_RENDER_NAME).join('');
}
...
export function createStagedRenders(baseDir) {
  ...
  // extra_json for the same reason gallery_shares.token_plain does: the listing selects
  // it on every page load, and nothing in this repo ever reads extra_json back. NULL is
  ...
  /**
   * Prepared search statements, keyed by term count.
   *
  ...
  /**
   * @param {number} terms - How many terms the query has (at least one).
   * @param {boolean} counting - The COUNT variant rather than the page.
   * @returns {any} A prepared statement.
   */
  function searchStatement(terms, counting) {
