...
export function createStagedRenders(baseDir) {
  ...
  const q = {
    ...
    // Same discipline as `rename` above, plus `status = 'ok'`. A row whose FIRST upload has
    // not finished must not be replaceable: its uploadInBackground is still in flight and
    // would markOk over the replacement moments later. Refusing here sends the caller down
    // the insert path instead, which is always safe.
    replaceResult: db.prepare(`
      UPDATE staged_renders SET width = ?, height = ?
      WHERE id = ? AND user_id = ? AND evicted_at IS NULL AND status = 'ok'
    `),
  ...
  return {
    ...
    /**
     * Swap one render's result bytes for a refined version, in place.
     *
     * The Masking Studio's "Looks Good" lands here when the photo it was given came out of
     * the gallery. NOTHING ABOUT THE ENTRY CHANGES — only the pixels and the dimensions
     * they imply. Everything on that list is load-bearing:
     *
     * - `created_at` is untouched, so a refine does not jump the render to the top of a
     *   gallery sorted by it. The owner refined a render; they did not make a new one.
     * - `custom_name` is untouched. If they named it, it keeps its name.
     * - `gallery_shares` is untouched — token, view count and settings all survive, and the
     *   link a buyer already has simply starts serving the better image.
     * - `extra_json` is untouched, so a refined INTERIOR render stays "Luxury Bedroom ·
     *   123-main" rather than being renamed "Masking Studio — 3 areas". "Masking Studio"
     *   names a render CREATED there, never one merely improved there.
     * - No eviction runs. No row is added, so the account's count is unchanged; running the
     *   cap here would delete somebody's oldest render as a side effect of an edit.
     * - `status` is not set. It is already `ok` — and the WHERE refuses anything else.
     * - The `before` blob is not passed and must not be: the ORIGINAL source photo is still
     *   the right "before" for a refined "after". Replacing it would make the slider
     *   compare the masking input against itself.
     *
     * Storage keys are unchanged, so the bytes are overwritten at the same paths. That is
     * why this takes byte counts rather than keys — and it is safe only because presigned
     * URLs are minted per request and nothing caches in front of the bucket. IF A CDN IS
     * EVER PUT IN FRONT OF R2, THIS BECOMES A STALENESS BUG and the keys need a version.
     *
     * @param {{ id: string, userId: string, width?: number, height?: number,
     *   blobs?: { role: string, storageKey: string, bytes?: number }[] }} arg - `userId` is
     *   the validated session id and is part of the WHERE, never a request body.
     * @returns {boolean} False when no live, finished row of the caller's matched — the
     *   caller then inserts a new entry instead, which is always safe.
     */
    replaceResult({ id, userId, width, height, blobs }) {
      return db.transaction(() => {
        const res = q.replaceResult.run(width ?? null, height ?? null, id, userId);
        if (res.changes === 0) return false;
        // Byte counts move with the bytes, or the account's usage drifts every refine.
        for (const b of blobs ?? []) q.insBlob.run(id, b.role, b.storageKey, b.bytes ?? 0, userId);
        return true;
      })();
    },
