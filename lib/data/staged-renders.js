 ...
 *
 * NOT advertised, and that is a deliberate product decision rather than an oversight:
 * Stagify+ is sold as unlimited STAGING, which it is — the cap here is on how many
 * finished renders the gallery keeps around as a convenience history, and 200 is far
 * past what anyone reaches. The pricing page says nothing about a gallery limit and
 * should not start.
 *
 * Where that call stops: an eviction that breaks a LIVE SHARE LINK is still reported to
 * the owner (see the `tier` field on the staging response and
 * public/scripts/app/gallery-notice.js). Not advertising a ceiling is a marketing
 * choice; letting a client's link die in silence is a broken product, and the two are
 * separable — the notice can say "a link stopped working" without ever mentioning why.
 */
