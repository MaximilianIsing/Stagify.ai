...
// Free accounts keep their last FREE_GALLERY_LIMIT entries. Checking the count and then
// inserting would be a read-modify-write across two statements, and two renders
// finishing at once would both see "19" and both insert. So the insert, the blob rows,
...
// not a pin: shared entries still COUNT against the cap, so a free user cannot grow
// their gallery by sharing everything. And when a free user has more live shares than
// the cap, eviction revokes the share in the same transaction rather than leaving a
...
export const FREE_GALLERY_LIMIT = Number(process.env.FREE_GALLERY_LIMIT) || 20;
...
export function createStagedRenders(baseDir) {
  ...
  // Read from SQL by the eviction query, so it cannot live in extra_json.
  ...
  const q = {
    ...
    graceFor: db.prepare('SELECT gallery_grace_until FROM users WHERE id = ?'),
    setGrace: db.prepare('UPDATE users SET gallery_grace_until = ? WHERE id = ?'),
  ...
  return {
    ...
    record({ render, blobs, isPro = false, now = Date.now() }) {
      ...
      const cap = isPro || inGracePeriod(render.userId, now) ? Infinity : FREE_GALLERY_LIMIT;
     ...
     * @param {{ userId: string, isPro?: boolean, now?: number }} arg
     * @returns {{ id: string, hadLiveShare: boolean }[]}
     */
    enforceCap({ userId, isPro = false, now = Date.now() }) {
      const cap = isPro || inGracePeriod(userId, now) ? Infinity : FREE_GALLERY_LIMIT;
      return db.transaction(() => evictBeyondCap(userId, cap, now))();
    },
