...
import { buildHreflangCluster, buildOgLocaleBlock, localizedPath, localizedUrl, LOCALIZED_PATHS } from './locales.js';
 ...
 * no `meta.keywords` fallback: pages whose block has no such property (guides.html's
 * BreadcrumbList, contact.html's ContactPage) must not have one invented for them,
 * which is exactly what the old fallback did — it stamped the *homepage's* keyword
 * list onto them. A page without the attribute keeps its authored JSON-LD untouched.
 *
 * @param {string} html
 * @param {Record<string, any>} translations
 * @returns {string}
...
function applyStructuredData(html, translations) {
  ...
  // Same tag the replace() below targets: the FIRST ld+json block in document order.
  const kwKey = (html.match(/<script[^>]*\btype="application\/ld\+json"[^>]*\bdata-lang-keywords="([^"]+)"/i) || [])[1];
