...
export function createUptimeMonitor(baseDir, options = {}) {
  ...
  function reset(now = Date.now()) {
    state = { version: STATE_VERSION, monitoringStart: now, lastBeat: now, bootCount: 1, incidents: [] };
    save();
  ...
  const api = { start, stop, beat, reset, getSnapshot, getStateFilePath: () => filePath, _getState: () => state };
  return api;
}
