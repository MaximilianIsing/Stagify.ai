...
import { withDisclosureMetadata } from './output-metadata.js';
...
// How far the nearest supported bucket may sit from the SOURCE ratio before pinning to it
// does more harm than good.
//
// The pin was applied unconditionally. Standard camera ratios land on a bucket exactly, but
// a CROPPED photo — routine in real-estate work, where horizons get straightened and
// doorways trimmed — does not: a 1620×1000 upload was pinned to 3:2, a 7.4% reshape, and at
// a bucket midpoint the gap reaches ~6% (3:2↔4:3), ~8.5% (16:9↔3:2), ~13% (21:9↔16:9). To
// satisfy a pin it cannot honour naturally, the model has to invent scene beyond the frame
// or throw scene away — and invented frame edges are exactly where a window becomes a wall.
// Worse, the prompt says "output the EXACT same aspect ratio as the input photo" at the same
// time, so the two instructions contradicted each other and the API config won.
//
// Below this, pinning is free and still buys the round-trip stability it was added for.
// Above it, the caller letterboxes instead (see resolveAspectRatioPin).
const AR_PIN_MAX_SOURCE_MISMATCH = 0.02;
...
// …but never let the long edge exceed this (px). Exported so callers that bound a
// resize REQUEST against what delivery could ever have produced (e.g. the download-result
// route's clampDim) share this ceiling instead of re-declaring their own copy of it.
export const DELIVERY_MAX_EDGE = 4096;
...
/**
 * The MIME type a buffer's bytes ACTUALLY are, for labelling a data URL.
 *
 * Exists because a model's output must never be labelled with its input's type.
 * The CAD path did exactly that — a JPEG blueprint produced
 * `data:image/jpeg;base64,<PNG bytes>` — and OpenAI vision validates the declared
 * type against the bytes, so annotation failed for every non-PNG blueprint (and
 * the user's download got the wrong extension). Sibling paths avoid this by
 * hard-coding the format they know they produced; this is for the case where the
 * format comes from the model and is not ours to assume.
 *
 * Falls back to image/png — the format every Gemini image model returns today —
 * rather than throwing, since a label is not worth failing a render over.
 *
 * @param {Buffer} buffer - Encoded image bytes.
 * @returns {Promise<string>} A MIME type such as 'image/png'.
 */
export async function detectImageMimeType(buffer) {
  try {
    const meta = await sharp(buffer).metadata();
    return meta && meta.format ? `image/${meta.format}` : 'image/png';
  } catch {
    return 'image/png';
  }
}
...
export async function upscaleForDelivery(dataUrl) {
  try {
    ...
    if (factor <= 1) {
      ...
      const webp = await withDisclosureMetadata(sharp(buffer), { mode: 'staged' })
...
/**
 * Decide whether to pin the model's output aspect ratio, and to what.
 *
 * `nearestGeminiAspectRatio` answers "which bucket is closest"; this answers the question
 * the callers actually have, which is "closest — but is it close ENOUGH to be worth
 * demanding?". A bucket more than AR_PIN_MAX_SOURCE_MISMATCH from the source is a request
 * to reshape the photograph, and a generative model satisfies that by inventing or
 * discarding the frame edges rather than by letterboxing politely.
 *
 * Returns the pin plus the source ratio the OUTPUT should ultimately be judged against —
 * they are the same number when pinned, and they are not when they aren't, which is the
 * distinction the crop safety net needs and previously did not have.
 *
 * @param {number} width - Source width in px (VISUAL/EXIF-oriented).
 * @param {number} height - Source height in px (VISUAL/EXIF-oriented).
 * @returns {{ label: string, ratio: number, sourceRatio: number } | null} The bucket to pin to (with the source ratio alongside), or null when no bucket fits closely enough / dimensions are missing.
 */
export function resolveAspectRatioPin(width, height) {
  if (!width || !height) return null;
  const sourceRatio = width / height;
  const nearest = nearestGeminiAspectRatio(width, height);
  if (!nearest) return null;
  const mismatch = Math.abs(nearest.ratio - sourceRatio) / sourceRatio;
  if (mismatch > AR_PIN_MAX_SOURCE_MISMATCH) {
    if (DEBUG_MODE) {
      logger.debug(`[AspectRatio] nearest bucket ${nearest.label} is ${(mismatch * 100).toFixed(1)}% from the source ratio — NOT pinning (would force the model to reshape the photo).`);
    }
    return null;
  }
  return { ...nearest, sourceRatio };
}
