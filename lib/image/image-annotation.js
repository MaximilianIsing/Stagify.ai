...
/**
 * Build the GPT-vision image-annotation API bound to the injected OpenAI client.
 * @param {{ openai: import('openai').default | null }} deps - Injected OpenAI client; annotation is skipped (returns null) when null.
 * @returns {{ annotateImage: (imageDataUrl: string, isCAD?: boolean, detectBlueprint?: boolean) => Promise<string | null> }} The annotation API.
 */
export function createImageAnnotation({ openai }) {
  /**
   * Describe an image in 5–10 words and classify it as CAD/blueprint or not, returning
   * "<description> CAD: True|False". Returns null when openai is null or on any error.
   * @param {string} imageDataUrl - The image as a data: URL.
   * @param {boolean} [isCAD=false] - Force CAD:True and skip blueprint detection.
   * @param {boolean} [detectBlueprint=false] - When false (and not isCAD), force CAD:False instead of asking the model to detect.
   * @returns {Promise<string | null>} The annotation string, or null on skip/error.
   */
  async function annotateImage(imageDataUrl, isCAD = false, detectBlueprint = false) {
    try {
      if (!openai) {
        if (DEBUG_MODE) {
          logger.debug('[Image Annotation] OpenAI not initialized, skipping annotation');
        }
        return null;
      }
      ...
      let annotation = messageContent.trim();
      ...
      if (cadMatch) {
        ...
        if (DEBUG_MODE) {
          logger.debug(`[Image Annotation] Warning: API did not return CAD classification, using default: ${isCAD ? 'True' : 'False'}`);
        }
      }
      
      if (DEBUG_MODE) {
        logger.debug(`[Image Annotation] Generated annotation: "${annotation}"`);
      }
      return annotation;
    } catch (error) {
      logger.error('[Image Annotation] Error annotating image:', error);
      return null;
    }
  }
