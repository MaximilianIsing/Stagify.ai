 ...
 * @property {boolean} [fullWidth] span the frame and sit flush to the bottom, instead of
 *   floating as a chip in the corner
 */

// THE FOUR LOOKS THE USER CAN PICK, and why each is built the way it is.
//
...
//
// `banner` is the other end of that argument, and the only one that is not a corner chip:
// a band across the foot of the frame, which is the most unmissable a disclosure gets short
// of writing across the room. It exists because the audiences differ — the agent who wants
// the badge to disappear and the agent whose MLS reviewer has already queried a photo are
// not asking for the same control, and one default cannot serve both. Note that this is
// exactly the "subtitle bar burnt into the photo" the RADIUS_RATIO note above rejects: as a
// DEFAULT it reads as somebody watermarking your listing, which is why it is not one. As a
// choice the user makes deliberately, it is the most compliant thing here.
...
export const STAMP_STYLES = {
  ...
  banner: {
    // Same horizontal padding as the chips, because it is measured from the TEXT: the band
    // keeps the glyphs exactly where the dark chip puts them, so switching style moves the
    // backdrop and not the words.
    padX: 0.95,
    padY: 0.42,
    text: [255, 255, 255],
    fill: [0, 0, 0],
    // A shade heavier than the chip's 0.52. The band is wide enough that a lighter wash
    // reads as a rendering artifact along the bottom edge rather than as a deliberate strip,
    // and unlike the chip it has no hairline to give it an edge — a light line here would
    // run the full width of the photo and look like a scan line.
    fillAlpha: 0.58,
    ring: [255, 255, 255],
    ringAlpha: 0,
    shadowAlpha: 0,
...
/**
 * Snap a browser-supplied badge configuration to the closed set this module can draw.
 *
 * FOUR call sites now ask the same question — /api/process-image and
 * /api/stage-by-endpoint-key (via virtual-staging-handler), /api/masking-studio/save,
 * /api/stamp-image and /api/disclosure-preview — because the badge is applied wherever the
 * finished pixels happen to be, and that is no longer one place. This is the single answer.
 *
 * EVERYTHING FALLS BACK; NOTHING REJECTS. The values arrive from a browser, and a stale or
 * hand-rolled one is not a reason to fail a render the user has already paid for: a badge in
 * the wrong style still discloses, and no badge does not. The language is validated against
 * the real locale set rather than trusted toward a filename, and the style against the
 * allow-list because it selects a code path.
 * @param {{ lang?: unknown, style?: unknown, scale?: unknown }} [options] - Raw request values.
 * @returns {{ lang: string, style: string, scale: number }} Values safe to render.
 */
export function normalizeStampOptions(options = {}) {
  ...
  return {
    ...
    scale: clampStampScale(options.scale),
  };
}

/**
 * Read the badge fields off a request body, in the `stamp*` shape the studios post.
 *
 * The flag is coerced rather than compared: it crosses a multipart boundary from
 * /api/process-image as the STRING "true", and a JSON boolean from the two studios.
 * @param {{ labelVirtuallyStaged?: unknown, stampLang?: unknown, stampStyle?: unknown, stampScale?: unknown }} [body] - The request body.
 * @returns {{ enabled: boolean, lang: string, style: string, scale: number }} The request's badge intent.
 */
export function readStampRequest(body = {}) {
  const flag = body.labelVirtuallyStaged;
  return {
    enabled: flag === true || flag === 'true' || flag === 'on',
    ...normalizeStampOptions({ lang: body.stampLang, style: body.stampStyle, scale: body.stampScale }),
  };
}
 ...
 * @returns {{ fontPx: number, textW: number, textH: number, padX: number, padY: number, pillW: number, pillH: number, radius: number, ring: number, margin: number, left: number, top: number, textLeft: number, textTop: number }} Placement in image pixels.
 */
export function badgeGeometry(imageW, imageH, masterW, masterH, options = {}) {
  ...
  // A full-width band is the frame's width, flush to its bottom edge, with square corners —
  // a margin or a radius on something that spans the photo reads as a floating panel that
  // failed to line up, not as a band. max(0, …) on the chip offsets keeps the composite
  // inside the frame even on a pathologically narrow image where the floored type size
  // still overflows; sharp rejects a negative offset.
  const pillW = style.fullWidth ? imageW : textW + padX * 2;
  const left = style.fullWidth ? 0 : Math.max(0, imageW - pillW - margin);
  const top = Math.max(0, imageH - pillH - (style.fullWidth ? 0 : margin));
  const radius = style.fullWidth ? 0 : Math.round(pillH * RADIUS_RATIO);

  // The GLYPHS land in the same place whatever the style, so switching between them moves
  // the backdrop and not the words: right-aligned, one margin plus one padding in from the
  // edge. Derived here rather than at the call site because the band's own left edge is the
  // far side of the photo, so `left + padX` — true for every chip — is nowhere near it.
  const textLeft = Math.max(0, imageW - margin - padX - textW);
  const textTop = top + padY;
