...
export function createImageReview({ genAI }) {
   ...
   * @param {{ instruction?: string, furnitureDataUrls?: string[], basePrompt?: string | null }} [opts] - Optional instruction to judge against, furniture reference data URLs to check for inclusion, and a rubric replacing QUALITY_REVIEW_PROMPT.
   * @returns {Promise<{ perfect: boolean, score: number, reason: string, degraded?: boolean }>} Verdict with a 0–100 score, the raw reviewer text, and `degraded` when the image was accepted without being reviewed.
   */
  async function reviewImageQuality(imageDataUrl, opts = {}) {
    ...
    try {
      const { instruction = '', furnitureDataUrls = [], basePrompt = null } = opts;
      ...
      /** @type {GeminiPart[]} */
      const parts = [{ text: (basePrompt || QUALITY_REVIEW_PROMPT) + instr + guide + REVIEW_WHY_SUFFIX }];
