...
export function createImageReview({ genAI }) {
   ...
   *
   * Same gate, different accept-list — and one hard difference: it can never reject with
   * VEHICLE. The interior taxonomy's `5` exists to catch someone uploading a photo of
   * their car; here a car on the driveway is the single most common thing the tool is
   * asked to remove, so rejecting on it would refuse the feature's canonical input.
   * See EXTERIOR_IGNORED_CODES for why that is enforced here rather than trusted to the
   * prompt's omission of the digit.
   * @param {Buffer} imageBuffer - The uploaded image bytes.
   * @returns {Promise<{ valid: boolean, code: string | null, reason: string }>} valid flag, the stable rejection code (null when valid), and the English rejection copy.
   */
