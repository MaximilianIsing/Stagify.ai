...
export function createConfig(deps) {
  ...
  function readFirstStripeFile(name, validate) {
    for (const dir of stripeSecretSearchDirs()) {
      ...
      try {
        ...
        logger.warn(`[stripe] Could not read ${name} in ${dir}:`, e.message);
      }
    }
    return null;
  }
  ...
  function readStripeSecretKey() {
    const fromFile = readFirstStripeFile('stripe_secret_key.txt', (raw) => {
      ...
      if (raw) logger.warn('[stripe] stripe_secret_key.txt must start with sk_ — ignored');
      return null;
    });
  ...
  function readStripeWebhookSecret() {
    const fromFile = readFirstStripeFile('stripe_webhook_secret.txt', (raw) => {
      ...
      if (raw) logger.warn('[stripe] stripe_webhook_secret.txt must start with whsec_ — ignored');
      return null;
    });
    if (fromFile) return fromFile;
  ...
  function readStripePublishableKey() {
    const fromFile = readFirstStripeFile('stripe_publishable.txt', (raw) => {
      ...
      if (raw) logger.warn('[stripe] stripe_publishable.txt must start with pk_ — ignored');
      return null;
    });
    if (fromFile) return fromFile;
  ...
  function readEnterprisePriceId() {
    const fromFile = readFirstStripeFile('priceid.txt', (raw) => {
      ...
      if (raw) logger.warn('[stripe] priceid.txt must contain a price_ id — ignored');
      return null;
    });
    if (fromFile) return fromFile;
  ...
  function readGoogleClientId() {
    const fromFile = readFirstStripeFile('googleclientID.txt', (raw) => {
      ...
      if (raw) logger.warn('[google] googleclientID.txt does not look like a Google OAuth client id — ignored');
      return null;
    });
    if (fromFile) return fromFile;
  ...
  function readApiCreditPriceIds() {
    /** @param {string} envName @param {string} fileName @returns {string} */
    const one = (envName, fileName) => {
      const fromFile = readFirstStripeFile(fileName, (raw) => {
        if (raw.startsWith('price_')) return raw;
        if (raw) logger.warn(`[stripe] ${fileName} must contain a price_ id — ignored`);
        return null;
      });
      if (fromFile) return fromFile;
    ...
    return {
      api_20: one('API_CREDIT_PRICE_20', 'api-credit-price-20.txt'),
      api_50: one('API_CREDIT_PRICE_50', 'api-credit-price-50.txt'),
      api_100: one('API_CREDIT_PRICE_100', 'api-credit-price-100.txt'),
      api_500: one('API_CREDIT_PRICE_500', 'api-credit-price-500.txt'),
    };
  }
