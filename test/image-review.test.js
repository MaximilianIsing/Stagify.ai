// GPT-vision QA reviewers (lib/image/image-review.js): reviewImageQuality,
...
// numeric SCORE is clamped to 0-100. We pin those contracts with a fake OpenAI (scripted
// content, no real call, no cost) and real sharp buffers/data-urls for the downscale step.
...
test('reviewImageQuality: not perfect keeps the parsed SCORE and clamps out-of-range values', async () => {
  const mid = await createImageReview({ openai: fakeOpenAI('PERFECT: false\nSCORE: 42') }).reviewImageQuality(TINY_PNG);
  assert.equal(mid.perfect, false);
  assert.equal(mid.score, 42);

  const over = await createImageReview({ openai: fakeOpenAI('SCORE: 150 — too generous') }).reviewImageQuality(TINY_PNG);
...
test('reviewImageQuality: a not-perfect verdict with no SCORE ranks as 0', async () => {
  const r = await createImageReview({ openai: fakeOpenAI('The lighting looks off but no number') }).reviewImageQuality(TINY_PNG);
  assert.equal(r.perfect, false);
  assert.equal(r.score, 0);
});

test('reviewImageQuality: a thrown API error fails open (accept the image)', async () => {
  const r = await createImageReview({ openai: fakeOpenAI(new Error('boom')) }).reviewImageQuality(TINY_PNG);
...
test('reviewImageQuality: extra furniture reference URLs that fail to downscale are skipped, not fatal', async () => {
  ...
  const r = await createImageReview({ openai: fakeOpenAI('PERFECT: true') })
...
test('reviewMaskEdit: disabled reviewer passes; parses score; fails open on error', async () => {
  assert.deepEqual(
    await createImageReview({ openai: null }).reviewMaskEdit(TINY_PNG, TINY_PNG),
    { perfect: true, score: 100, reason: 'reviewer disabled' },
  );

  const good = await createImageReview({ openai: fakeOpenAI('PERFECT: true') }).reviewMaskEdit(TINY_PNG, TINY_PNG);
  assert.equal(good.perfect, true);

  const scored = await createImageReview({ openai: fakeOpenAI('PERFECT: false\nSCORE: 73') })
  ...
  const failed = await createImageReview({ openai: fakeOpenAI(new Error('nope')) }).reviewMaskEdit(TINY_PNG, TINY_PNG);
...
test('validateStageableImage: "VALID: false | REASON: ..." surfaces the reason (quotes stripped)', async () => {
  const { validateStageableImage } = createImageReview({ openai: fakeOpenAI('VALID: false\nREASON: "This is a person, not a room"') });
...
test('validateStageableImage: a thrown error fails open (allow the upload)', async () => {
  const { validateStageableImage } = createImageReview({ openai: fakeOpenAI(new Error('vision down')) });
  assert.deepEqual(await validateStageableImage(await roomBuffer()), { valid: true, reason: '' });
});
